# The setup form: every question Hexarchy asks a human to describe their machine
...
# (hexarchy-provision-owner). Sourced by both, so the copies cannot drift the way
...
HEXARCHY_FORM_BACK=1
HEXARCHY_FORM_SIGNAL=130
...
HEXARCHY_KEYBOARD_LAYOUTS=$'English (US)|us
English (UK)|uk
...
HEXARCHY_USERNAME_PATTERN='^[a-z_][a-z0-9_-]*[$]?$'
HEXARCHY_RESERVED_USERNAMES='^(root|bin|daemon|mail|ftp|http|nobody|dbus|systemd-coredump|systemd-network|systemd-oom|systemd-journal-remote|systemd-resolve|systemd-timesync|tss|uuidd|alpm|git|avahi|cups|lp|_talkd|polkitd|rtkit|qemu|brltty|gluster|rpc|libvirt-qemu|pcscd|nvidia-persistenced|sddm)$'
HEXARCHY_HOSTNAME_PATTERN='^[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?$'
HEXARCHY_HOSTNAME_DEFAULT='hexarchy'
...
hexarchy_username_taken() { return 1; }
...
hexarchy_prompt_keyboard() {
  local choice status
  choice=$(printf '%s\n' "$HEXARCHY_KEYBOARD_LAYOUTS" | cut -d'|' -f1 |
    gum choose --height 10 --selected "English (US)" --header "Select keyboard layout") && status=0 || status=$?
  ((status == 0)) || return $status

  keyboard_label="$choice"
  keyboard=$(printf '%s\n' "$HEXARCHY_KEYBOARD_LAYOUTS" | awk -F'|' -v c="$choice" '$1==c{print $2; exit}')
}

hexarchy_prompt_username() {
  local status
  while true; do
    ...
    if [[ "$username" =~ $HEXARCHY_USERNAME_PATTERN ]]; then
      if [[ "$username" =~ $HEXARCHY_RESERVED_USERNAMES ]]; then
        notice "Username is reserved for system" 1
      elif hexarchy_username_taken "$username"; then
...
hexarchy_prompt_hostname() {
  local status
  while true; do
    hostname=$(gum input --placeholder "Letters, digits, and dashes (or return for 'hexarchy')" --prompt.foreground="#845DF9" --prompt "Hostname> ") && status=0 || status=$?
    ((status == 0)) || return $status

    if [[ -z $hostname ]]; then
      hostname="$HEXARCHY_HOSTNAME_DEFAULT"
      return 0
    elif [[ "$hostname" =~ $HEXARCHY_HOSTNAME_PATTERN ]]; then
      return 0
    else
