...
if hexarchy-hw-match "XPS" && hexarchy-hw-intel-ptl; then
  echo "Detected Dell XPS Panther Lake, installing PTL kernel..."

  hexarchy-pkg-add linux-ptl linux-ptl-headers
  ...
  # Named to sort after hexarchy-defaults.conf: drop-ins are read in order and
