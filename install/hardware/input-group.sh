...
# first boot instead).
provisioning_dir="${HEXARCHY_PROVISIONING_DIR:-/var/lib/hexarchy/provisioning}"
mkdir -p "$provisioning_dir"
grep -qxF input "$provisioning_dir/groups" 2>/dev/null || echo input >>"$provisioning_dir/groups"

if [[ -n ${HEXARCHY_INSTALL_USER:-} ]] && getent passwd "$HEXARCHY_INSTALL_USER" >/dev/null; then
  usermod -aG input "$HEXARCHY_INSTALL_USER"
fi
