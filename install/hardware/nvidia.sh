if lspci | grep -qi 'nvidia'; then
  ...
  [[ -n $KERNEL_PACKAGE ]] && hexarchy-pkg-add "$KERNEL_PACKAGE-headers"

  if hexarchy-hw-nvidia-gsp; then
    PACKAGES=(nvidia-open-dkms nvidia-utils lib32-nvidia-utils libva-nvidia-driver)
  elif hexarchy-hw-nvidia-without-gsp; then
