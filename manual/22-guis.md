...
You can open the Share menu on `Super + Ctrl + S` or under _Trigger > Share_ in the Hexarchy menu. It gives you four options:
...
The same thing works from the terminal with `hexarchy share clipboard`, `hexarchy share file [path]`, and `hexarchy share folder [path]`. Leave the path off and you get the picker.

You can also send straight from the file manager: right-click any selection in Nautilus and pick _Send via LocalSend_.

Hexarchy's firewall is closed by default except for LocalSend's port, so this works out of the box on a fresh install. See [security](48-security.md).
...
You start Signal with `Super + Shift + G`. It's not part of the base install, so the first time you hit that, Hexarchy will offer to install it for you (it's also under _Install > Service_ in the Hexarchy menu).
...
[OBS Studio](https://obsproject.com/) lets you record or stream video from multiple inputs. You can mix a screencast with a webcam with a microphone input. It's what was used to record the Hexarchy screencasts.
