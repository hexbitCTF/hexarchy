...
# rewrite the host), so Hexarchy no longer puts users in it by default. Bring
...
# it from prompting mid-update — hexarchy-update-restart handles the reboot once
# the whole update has finished.
if id -nG "$USER" | grep -qw docker; then
  HEXARCHY_DEFER_REBOOT=1 hexarchy-remove-security-sudoless-docker
fi
...
if [[ -f $dest ]]; then
  cp "$HEXARCHY_PATH/applications/Docker.desktop" "$dest"
fi
