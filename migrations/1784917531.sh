...
# race is fixed upstream. The packaged hexarchy-defaults.conf now carries the
# same parameter for fresh installs.

if hexarchy-cmd-present limine-mkinitcpio &&
  [[ -f /etc/limine-entry-tool.d/hexarchy-defaults.conf ]] &&
  ! grep -rqs "initramfs_async" /etc/limine-entry-tool.d/ /etc/default/limine; then
  echo 'KERNEL_CMDLINE[default]+=" initramfs_async=0"' |
    sudo tee -a /etc/limine-entry-tool.d/hexarchy-defaults.conf >/dev/null
