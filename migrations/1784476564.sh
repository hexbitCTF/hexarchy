...
# hexarchy_hooks.conf now applies the same condition on every rebuild.

hooks_conf="/etc/mkinitcpio.conf.d/hexarchy_hooks.conf"

# vconsole.conf only guarantees KEYMAP, and it need not exist at all. Sourcing
# one that isn't there fails, and under hexarchy-migrate's `bash -euo pipefail`
...
if [[ $layout =~ ^(af|am|ara|bd|bg|by|et|ge|gr|il|in|iq|ir|kg|kh|kz|la|lk|mk|mm|mn|mv|np|rs|ru|sy|th|tj|ua)$ ]] &&
  ...
  if hexarchy-cmd-present limine-mkinitcpio; then
