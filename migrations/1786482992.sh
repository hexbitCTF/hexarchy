...
# config, but hexarchy-settings ships hexarchy-defaults.conf with that parameter
...
# landed. Such a machine boots without any of hexarchy-defaults.conf's command
# line — including initramfs_async=0, so Plymouth still loses the LUKS prompt.

defaults_conf="${HEXARCHY_LIMINE_DEFAULTS_CONF:-/etc/limine-entry-tool.d/hexarchy-defaults.conf}"
running_cmdline="${HEXARCHY_RUNNING_CMDLINE:-/proc/cmdline}"
rebuild_marker="${HEXARCHY_LIMINE_REBUILD_MARKER:-/var/lib/hexarchy/migrations/1786482992}"
