...
# The widget formerly known as hexarchy.model-usage is now hexarchy.agents, and
# it no longer scans providers itself: it displays the records that
# hexarchy-agent-usage-update writes under ~/.local/state/hexarchy/agents/usage/.
...
if [[ -s $config_file ]]; then
  ...
  jq '
    def rename:
      if . == "hexarchy.model-usage" then
        "hexarchy.agents"
      elif type == "object" and .id == "hexarchy.model-usage" then
        .id = "hexarchy.agents"
      else
        .
      end;
...
rm -rf "$HOME/.cache/hexarchy/model-usage"

hexarchy-agent-usage-update || true
