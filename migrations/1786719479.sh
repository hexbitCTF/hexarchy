...
agent_file="$HOME/.config/hexarchy/defaults/agent"
skills_source="$HEXARCHY_PATH/default/agents/skills"

# Read the default the way hexarchy-default-agent reads it, so this migration and
...
if hexarchy-cmd-missing agy &&
  { [[ $selected_agent == "gemini" ]] || [[ ! -f $HOME/.local/state/hexarchy/preinstalls-removed ]]; }; then
  hexarchy-mise-install antigravity-cli agy
fi
...
# Remove Preinstalls no longer lists gemini, so Hexarchy's own wrapper would
