...
# hexarchy_hooks.conf now filters the kms hook out of HOOKS when the proprietary
...
# dropped out. A user-edited hexarchy_hooks.conf (pacman leaves the packaged
# update as a .pacnew) keeps kms and correctly skips the rebuild.

hooks_conf="${HEXARCHY_MKINITCPIO_HOOKS_CONF:-/etc/mkinitcpio.conf.d/hexarchy_hooks.conf}"
nvidia_conf="${HEXARCHY_MKINITCPIO_NVIDIA_CONF:-/etc/mkinitcpio.conf.d/nvidia.conf}"
rebuild_marker="${HEXARCHY_KMS_REBUILD_MARKER:-/var/lib/hexarchy/migrations/1786605598}"

hexarchy-cmd-present limine-mkinitcpio || exit 0
