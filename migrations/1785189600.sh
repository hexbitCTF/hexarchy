...
# Drop only the hooks Hexarchy installed, wherever the migrations that wrote
...
if [[ -f $tmux_config ]]; then
  ...
  if awk '
    ...
    /^[[:space:]]*set-hook -g alert-(bell|activity|silence) .*hexarchy\.indicators refresh/ ||
    /^[[:space:]]*set-hook -g (after-select-window|client-session-changed|client-focus-(in|out))(\[[0-9]+\])? .*(hexarchy-tmux-alert track|hexarchy\.indicators refresh)/ {
      header = ""
      dropped = 1
      next
    }
...
if tmux has-session 2>/dev/null; then
  while read -r hook; do
    ...
    awk '$0 ~ /hexarchy-tmux-alert|hexarchy\.indicators refresh/ { print $1 }')

  # The removed track subcommand stamped this on every window it saw.
  while read -r window; do
    [[ -n $window ]] || continue
    tmux set-option -wqu -t "$window" @hexarchy_unfocused_activity 2>/dev/null || true
...
if [[ -s $config_file ]] && grep -q 'TmuxAlert' "$config_file"; then
  ...
  if jq '
    ...
    def cleaned:
      if type == "object" and .id == "hexarchy.indicators" then stripped("items") | stripped("indicators") else . end;

    def emptied:
      type == "object" and .id == "hexarchy.indicators" and
