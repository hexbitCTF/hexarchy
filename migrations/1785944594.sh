...
limine_conf="${HEXARCHY_T2_LIMINE_CONF:-/etc/limine-entry-tool.d/t2-mac.conf}"
fan_conf="${HEXARCHY_T2_FAN_CONF:-/etc/t2fand.conf}"
running_cmdline="${HEXARCHY_T2_RUNNING_CMDLINE:-/proc/cmdline}"
repair_marker="${HEXARCHY_T2_REPAIR_MARKER:-/var/lib/hexarchy/migrations/1785944594}"
...
if hexarchy-pkg-present tiny-dfr; then
  sudo systemctl disable --now tiny-dfr.service || true
  hexarchy-pkg-drop tiny-dfr
fi
