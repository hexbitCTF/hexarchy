echo "Switch mise to the mise-bin package from the Hexarchy repo"
...
# hexarchy-zsh and hexarchy-fish, which depend on it; the provides is what keeps
# them satisfied when mise-bin lands in the same transaction that drops mise.
#
# hexarchy-pkg-add cannot do that swap: pacman answers its own conflict question
...
if hexarchy-pkg-missing mise-bin; then
