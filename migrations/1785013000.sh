...
zram_conf="${HEXARCHY_ZRAM_CONF:-/etc/systemd/zram-generator.conf}"
zram_dropin="${HEXARCHY_ZRAM_DROPIN:-/usr/lib/systemd/zram-generator.conf.d/90-hexarchy.conf}"

# The tuning ships as /usr/lib/systemd/zram-generator.conf.d/90-hexarchy.conf.
...
# that sets nothing at all, which hexarchy-migrate's -e would take as a failed
...
if [[ -z $settings || $settings =~ ^\[zram0\]compression-algorithm=[[:alnum:]-]+$ ]]; then
  ...
  echo "Hexarchy's drop-in overrides it. Move your changes to"
