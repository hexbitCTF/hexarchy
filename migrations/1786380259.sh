...
marker="${HEXARCHY_BLUETOOTH_MIGRATION_MARKER:-/var/lib/hexarchy/migrations/1786380259}"
main_conf="${HEXARCHY_BLUETOOTH_MAIN_CONF:-/etc/bluetooth/main.conf}"
...
if hexarchy-bluetooth-power is-on; then
  sudo hexarchy-bluetooth-power on
else
  sudo hexarchy-bluetooth-power off
fi

# Hexarchy set AutoEnable=false believing bluetoothd would then restore the last
...
# Hexarchy wrote is reverted, so a hand-edited opt-out survives.
if [[ -f $main_conf ]]; then
