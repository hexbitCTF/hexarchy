...
rev 3 names the feature **dots** (`hexarchy dots ...`) and adds the
multi-machine sync design.

## Problem

Hexarchy declares `~/.config` "your files" but does little to preserve them:

- `hexarchy-refresh-config` litters `*.bak.<timestamp>` files next to originals.
- `hexarchy-reinstall-configs` clobbers everything back to `/etc/skel` defaults.
...
- **`~/.config/hexarchy` only**: too narrow; misses hypr, terminals, `.bashrc`.
...
- **Raw git passthrough / lazygit over `$HOME`** (rejected in review):
  ...
  raw invocation themselves; Hexarchy will not bless it.
...
```bash
git init --bare ~/.local/share/hexarchy/dots.git   # mode 0700
```
...
- Repo-local config only: synthetic identity (`Hexarchy <hexarchy@localhost>`),
...
- Never export `GIT_DIR`/`GIT_WORK_TREE` (would leak into `hexarchy-plugin-*`,
  `hexarchy-theme-update`, `hexarchy-update-dev`, user hooks).
...
- **Best-effort everywhere**: a failed snapshot warns and continues. History
  ...
  `hexarchy-update`/`hexarchy-migrate` (`set -e` there must not see git errors).

### Audited manifest, not a derived whitelist

Tracking is `git add -f --pathspec-from-file=<manifest>` only. The manifest is
a hand-audited file shipped with Hexarchy — explicitly **not** derived from
`$HEXARCHY_PATH/config` (which ships Chromium Preferences, fcitx5,
`opencode/opencode.json` where users put API keys, etc.):

- Include: `.config/hypr/*.{lua,conf}`, `.config/hexarchy/shell.json`,
  `.config/hexarchy/extensions/**`, `.config/hexarchy/hooks/**`,
  terminal configs (alacritty/foot/ghostty/kitty), `.config/btop/btop.conf`,
  `.config/starship.toml`, `.bashrc`, `.XCompose`.
- Exclude (deliberately): `.config/hexarchy/plugins/**` and
  `.config/hexarchy/themes/**` (nested git clones managed by
  `hexarchy-plugin-*` / `hexarchy-theme-update`), backgrounds (multi-MB
...
- At seed time and before each snapshot: if manifest paths are symlinks or a
  ...
  detected, mark the repo dormant and say so in `hexarchy dots status`.
...
internal `hexarchy-dots-snapshot "<label>"`:

1. **`hexarchy-provision-user`**: seed repo + initial commit, after `git.sh`
   and `.XCompose` are in place. Idempotent; `--force` must not re-init.
2. **`hexarchy-migrate`**: before/after pair labeled with version. Covers both
   `hexarchy-update` *and* the login-time migration path, and separates
...
3. **Batch refresh commands** (`hexarchy-refresh-hyprland` etc.): one pair per
   command, not one commit per `hexarchy-refresh-config` call (hyprland calls
   it seven times).
4. **`hexarchy-reinstall-configs`**: a "before" snapshot — honestly labeled.
...
5. **Timer**: an hourly-ish systemd user timer committing only if dirty, so
   ...
   fails for any edit not followed by an Hexarchy operation.
...
- **`hexarchy dots push`** — *squash-publishes* the current manifest state
...
- **`hexarchy dots pull [url]`** — fetches `sync`, takes a local pre-pull
...
one `hexarchy dots restore` away.

Remote setup UX:

- `hexarchy dots push` with no remote runs setup inline: paste a git URL, or
...
- Dormant mode (dotfile-manager users) disables sync too — they have their
  own.
- v2 candidate: `hexarchy-provision-first-run` asks "Got a dots repo?" so a
...
invoked as `hexarchy dots <verb>`:

- `hexarchy dots snapshot [label]` — take a snapshot now
- `hexarchy dots log` — history with labels
- `hexarchy dots diff [ref]` — default range: the last before/after pair
- `hexarchy dots restore <file> [--at <ref>]` — accepts `~/.config/...`,
...
- `hexarchy dots push` / `hexarchy dots pull [url]` — sync (above)
- `hexarchy dots status` — repo state, dormant/active, remote, last
  snapshot/publish
...
- New users: seeded by `hexarchy-provision-user`.
...
- Docs/tests alignment: manual `31-dotfiles.md` gets the built-in story
  ...
  per the documentation layout; `default/agents/skills/hexarchy/SKILL.md`
...
1. Manifest location: `default/hexarchy/dots-manifest` vs alongside the
   helper; and whether users may extend it via
   `~/.config/hexarchy/dots-manifest.d/`.
2. Whether `.config/hexarchy/hooks/**` belongs in the manifest at all (small
   scripts, but the likeliest place for a pasted token).
3. Harden `hexarchy-refresh-config` against `..` path escape (documented today
