...
modeless_lock="$test_tmp/hexarchy-monitor-modeless.lock"
...
if [[ ${1:-} == "monitors" && ${2:-} == "all" && ${3:-} == "-j" ]]; then
  [[ -f $HEXARCHY_TEST_HYPRCTL_FAIL_FLAG ]] && exit 4
  cat "$HEXARCHY_TEST_MONITORS_FILE"
elif [[ ${1:-} == "reload" ]]; then
  printf 'reload\n' >>"$HEXARCHY_TEST_RELOAD_LOG"
  [[ -f $HEXARCHY_TEST_RECOVERED_MONITORS ]] &&
    cp "$HEXARCHY_TEST_RECOVERED_MONITORS" "$HEXARCHY_TEST_MONITORS_FILE"
fi
SH

cat >"$fake_bin/hexarchy-hyprland-reload-guard" <<'SH'
#!/bin/bash

[[ ${1:-} == "paused" && ${HEXARCHY_TEST_GUARD_PAUSED:-0} == 1 ]]
SH

cat >"$fake_bin/socat" <<'SH'
#!/bin/bash

exec cat "$HEXARCHY_TEST_EVENTS"
SH

# A desktop, so the clamshell half of the watcher stays idle.
cat >"$fake_bin/hexarchy-hw-laptop" <<'SH'
#!/bin/bash

exit 1
SH

cat >"$fake_bin/hexarchy-hyprland-monitor-clamshell" <<'SH'
#!/bin/bash
...
chmod +x "$fake_bin"/*
ln -s "$ROOT/bin/hexarchy-hyprland-monitor-modeless" "$fake_bin/hexarchy-hyprland-monitor-modeless"
...
assert_state() {
  ...
  PATH="$fake_bin:$PATH" HEXARCHY_TEST_MONITORS_FILE="$monitors_file" \
  HEXARCHY_TEST_HYPRCTL_FAIL_FLAG="$hyprctl_fail" \
    "$ROOT/bin/hexarchy-hyprland-monitor-modeless" || actual=$?
...
PATH="$fake_bin:$PATH" HEXARCHY_TEST_MONITORS_FILE="$monitors_file" \
HEXARCHY_TEST_HYPRCTL_FAIL_FLAG="$hyprctl_fail" \
  "$ROOT/bin/hexarchy-hyprland-monitor-modeless" || unreachable=$?
rm -f "$hyprctl_fail"
...
start_watcher() {
  ...
  HEXARCHY_TEST_EVENTS="$events" \
  HEXARCHY_TEST_MONITORS_FILE="$monitors_file" \
  HEXARCHY_TEST_RELOAD_LOG="$reload_log" \
  HEXARCHY_TEST_RECOVERED_MONITORS="$recovered" \
  HEXARCHY_TEST_GUARD_PAUSED="${1:-0}" \
  HEXARCHY_TEST_HYPRCTL_FAIL_FLAG="$hyprctl_fail" \
    "$ROOT/bin/hexarchy-hyprland-monitor-watch" &
  watch_pid=$!
