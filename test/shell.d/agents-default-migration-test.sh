...
home="$test_dir/home"
config="$home/.config/hexarchy/shell.json"
...
write_config() {
  rm -rf "$home"
  mkdir -p "$home/.config/hexarchy"
  jq "${1:-.}" "$ROOT/config/hexarchy/shell.json" >"$config"
}

without_widget='del(.bar.layout[][] | select((if type == "object" then .id else . end) == "hexarchy.agents"))'
...
jq -e '[.bar.layout.right[].id] | index("hexarchy.agents")' "$ROOT/config/hexarchy/shell.json" >/dev/null ||
...
[[ $(ids right) == '["hexarchy.tray","hexarchy.agents","hexarchy.bluetooth","hexarchy.network","hexarchy.audio","hexarchy.monitor","hexarchy.power"]' ]] ||
  fail "migration inserts the agents widget after the tray" "$(ids right)"
pass "migration inserts the agents widget after the tray"

(($(wc -l <"$SHELL_RESTARTS") == 0)) || fail "migration leaves the shell restart to hexarchy update"
pass "migration leaves the shell restart to hexarchy update"
...
write_config "$without_widget | .bar.layout.center += [{ id: \"hexarchy.agents\" }]"
run_migration

[[ $(ids center) == *'"hexarchy.agents"'* ]] || fail "migration leaves a user-placed widget alone" "$(ids center)"
[[ $(ids right) != *'"hexarchy.agents"'* ]] || fail "migration does not add a second copy" "$(ids right)"
pass "migration respects a widget the user already placed"

# Layouts written before entries grew options are bare id strings.
write_config "$without_widget | .bar.layout.right = [\"hexarchy.tray\", \"hexarchy.agents\", \"hexarchy.power\"]"
run_migration

[[ $(ids right) == '["hexarchy.tray","hexarchy.agents","hexarchy.power"]' ]] ||
...
write_config "$without_widget | del(.bar.layout.right[] | select(.id == \"hexarchy.tray\"))"
run_migration

[[ $(ids right) == '["hexarchy.agents",'* ]] || fail "migration places the widget without a tray" "$(ids right)"
