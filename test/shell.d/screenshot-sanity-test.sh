...
jq -e '
  any(.[]; .id == "hexarchy.menu" and .visible == true and .width > 0 and .height > 0) and
  any(.[]; .id == "hexarchy.clock" and .visible == true and .width > 0 and .height > 0)
' <<<"$geometry" >/dev/null || {
  printf 'Geometry:\n' >&2
  jq . <<<"$geometry" >&2
...
screenshot=$(
  HEXARCHY_PATH="$test_root" \
  HEXARCHY_SCREENSHOT_DIR="$screenshot_dir" \
  HOME="$test_home" \
  PATH="$stub_bin:$ROOT/bin:$PATH" \
    "$ROOT/bin/hexarchy" capture screenshot fullscreen save 2>"$screenshot_err" | tail -n 1
)
