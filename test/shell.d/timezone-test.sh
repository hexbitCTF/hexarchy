...
timezone_menu="$ROOT/bin/hexarchy-menu-timezone"
sudoers_file="$ROOT/etc/sudoers.d/hexarchy-tzupdate"
...
grep -F 'hexarchy-shell -q hexarchy.clock refresh' "$timezone_menu" >/dev/null ||
  fail "timezone menu refreshes the namespaced clock IPC target"

! grep -F 'hexarchy-shell -q Clock refresh' "$timezone_menu" >/dev/null ||
