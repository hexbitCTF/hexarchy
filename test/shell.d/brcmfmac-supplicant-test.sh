...
cat >"$stub_bin/hexarchy-state" <<'SH'
#!/bin/bash

printf 'hexarchy-state' >>"$TEST_LOG"
printf '\t%s' "$@" >>"$TEST_LOG"
printf '\n' >>"$TEST_LOG"
SH
...
run_migration() {
  ...
  WIFI_ID="$wifi_id" T2_HARDWARE="$t2" PATH="$stub_bin:$PATH" TEST_LOG="$calls" \
    HEXARCHY_BRCMFMAC_DMI_VENDOR="$test_tmp/dmi/sys_vendor" \
    HEXARCHY_BRCMFMAC_CONF="$conf" \
...
grep -Fq $'hexarchy-state\tset\treboot-required' "$calls" ||
