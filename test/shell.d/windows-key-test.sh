...
command="$ROOT/bin/hexarchy-windows-key"
tmp=$(mktemp -d)
trap 'rm -rf "$tmp"' EXIT
...
run_helper() {
  HEXARCHY_MSDM_PATH=$1 PATH="$ROOT/bin:$PATH" "$command"
}
...
output=$(PATH="$tmp/bin:$ROOT/bin:$PATH" HEXARCHY_MSDM_PATH="$tmp/msdm" "$command")
...
if HEXARCHY_MSDM_PATH="$tmp/empty" PATH="$ROOT/bin:$PATH" "$command" >"$tmp/out" 2>"$tmp/err"; then
...
if HEXARCHY_MSDM_PATH="$tmp/missing" PATH="$ROOT/bin:$PATH" "$command" >"$tmp/out" 2>"$tmp/err"; then
