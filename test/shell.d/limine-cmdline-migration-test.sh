...
packaged_defaults="$ROOT/etc/limine-entry-tool.d/hexarchy-defaults.conf"
...
defaults_conf="$test_tmp/hexarchy-defaults.conf"
...
run_migration() {
  PATH="$stub_bin:$PATH" \
    TEST_LOG="$calls" \
    HEXARCHY_LIMINE_DEFAULTS_CONF="$defaults_conf" \
    HEXARCHY_RUNNING_CMDLINE="$running_cmdline" \
    HEXARCHY_LIMINE_REBUILD_MARKER="$rebuild_marker" \
