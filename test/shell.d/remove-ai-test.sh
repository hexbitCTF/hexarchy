...
cat >"$tmp_dir/bin/hexarchy-pkg-drop" <<'SCRIPT'
#!/bin/bash
printf 'drop:%s\n' "$*" >>"$TEST_LOG"
SCRIPT
chmod +x "$tmp_dir/bin/hexarchy-pkg-drop"
...
"$ROOT/bin/hexarchy-remove-ai-chatgpt" >/dev/null
...
"$ROOT/bin/hexarchy-remove-ai-lm-studio" >/dev/null
...
"$ROOT/bin/hexarchy-remove-ai-t3-code" >/dev/null
...
# ~/.grok belongs to the Grok CLI that hexarchy-default-agent installs.
fresh_home
mkdir -p "$HOME/.config/Grok Bot" "$HOME/.grokbot" "$HOME/.grok"
"$ROOT/bin/hexarchy-remove-ai-grok-bot" >/dev/null
...
# builds hexarchy-pkg-drop will not touch.
ollama_row=$(grep '^  "remove.ai.ollama":' "$ROOT/default/hexarchy/hexarchy-menu.jsonc")
[[ $ollama_row == *'"when":"hexarchy-pkg-present ollama"'* ]] ||
