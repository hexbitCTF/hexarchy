...
cat >"$TMPDIR/bin/hexarchy-notification-send" <<'SH'
#!/bin/bash
exit 0
SH

chmod +x "$TMPDIR/bin/systemctl" "$TMPDIR/bin/hexarchy-notification-send"

test_home="$TMPDIR/home"
flag="$test_home/.local/state/hexarchy/toggles/crash-capture-off"

toggle_crash_capture() {
  PATH="$TMPDIR/bin:$ROOT/bin:$PATH" \
  SYSTEMCTL_LOG="$SYSTEMCTL_LOG" \
  HOME="$test_home" \
    "$ROOT/bin/hexarchy-toggle-crash-capture"
}

toggle_crash_capture
[[ -f $flag ]] || fail "crash capture toggle disables the watcher"
grep -Fqx -- "--user stop hexarchy-crash-watch.service" "$SYSTEMCTL_LOG" ||
...
grep -Fqx -- "--user start hexarchy-crash-watch.service" "$SYSTEMCTL_LOG" ||
...
service="$ROOT/default/systemd/user/hexarchy-crash-watch.service"
grep -Fx 'ConditionPathExists=!%h/.local/state/hexarchy/toggles/crash-capture-off' "$service" >/dev/null ||
...
grep -F 'hexarchy-crash-watch.service' "$ROOT/install/user/first-run/enable-user-units.sh" >/dev/null ||
...
const items = menu.parseMenuJsonc(fs.readFileSync(path.join(root, 'default/hexarchy/hexarchy-menu.jsonc'), 'utf8'))
const byId = Object.fromEntries(items.map(item => [item.id, item]))

assertEqual(
  byId['trigger.toggle.crash-capture'].action,
  'hexarchy-toggle-crash-capture',
