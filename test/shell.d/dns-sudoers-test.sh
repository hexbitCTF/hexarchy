...
dns="$ROOT/bin/hexarchy-dns"
sudoers_file="$ROOT/etc/sudoers.d/hexarchy-dns"
rule='%wheel ALL=(root) NOPASSWD: /usr/bin/hexarchy-dns Cloudflare, /usr/bin/hexarchy-dns Google, /usr/bin/hexarchy-dns DHCP'
...
grep -Fx 'PACKAGED_PATH=/usr/bin/hexarchy-dns' "$dns" >/dev/null ||
  fail "hexarchy-dns elevates the path the sudoers rule names"

# `sudo -l` on its own answers whether a command is permitted, not whether it
# is passwordless, and Hexarchy ships a blanket %wheel rule that permits
# everything. Only the long listing prints the matched entry's tags.
grep -E 'sudo -n -l -l' "$dns" >/dev/null ||
  fail "hexarchy-dns reads the grant from the long sudo listing"
...
# (etc/sudoers.d/hexarchy-dev-path) prepends a user-writable checkout bin/ to it.
...
grep -Eq '^\s*export PATH=/usr/local/sbin:/usr/local/bin:/usr/bin' "$dns" ||
  fail "hexarchy-dns pins PATH to trusted system directories when it holds root"
...
[[ $gated == *"export PATH=/usr/local/sbin:/usr/local/bin:/usr/bin"* ]] ||
  fail "hexarchy-dns gates the trusted-PATH pin on holding root"
...
SH
  ...
  if ! PATH="$poison_dir:$PATH" "${root_runner[@]}" bash "$dns" </dev/null >/dev/null 2>&1; then
    rm -rf "$poison_dir"
    fail "root hexarchy-dns failed its read-only trusted-PATH probe"
  fi
  if [[ -e $poison_ran ]]; then
    rm -rf "$poison_dir"
    fail "root hexarchy-dns resolved a bare helper from the front of PATH instead of a trusted system path"
  fi
  rm -rf "$poison_dir"
  pass "root hexarchy-dns resolves system helpers from a trusted PATH, not the invocation PATH"
else
...
# STUB_GRANTED, the providers etc/sudoers.d/hexarchy-dns covers on this machine,
# and logs the elevation otherwise. STUB_GRANTED empty stands for an install
# whose hexarchy-settings predates the file.
...
for provider in Cloudflare Google DHCP; do
  ...
  [[ $elevation == "sudo /usr/bin/hexarchy-dns $provider" ]] ||
    fail "hexarchy-dns takes the passwordless sudo grant for $provider without a terminal" "got: $elevation"
done

pass "hexarchy-dns elevates the stock providers through sudo, not polkit"
...
dev_linked=$(HEXARCHY_PATH="$test_tmp/checkout" elevation_for Cloudflare)
[[ $dev_linked == "sudo /usr/bin/hexarchy-dns Cloudflare" ]] ||
  fail "hexarchy-dns elevates the system install wherever HEXARCHY_PATH points" "got: $dev_linked"

custom=$(elevation_for Custom)
[[ $custom == "pkexec /usr/bin/hexarchy-dns Custom" ]] ||
  fail "hexarchy-dns leaves Custom on the polkit path, since no sudoers rule covers it" "got: $custom"

# The grant is what makes sudo passwordless, so its absence -- an install still
# on an older hexarchy-settings, or a user outside %wheel the rule cannot match
...
[[ $ungranted == "pkexec /usr/bin/hexarchy-dns Cloudflare" ]] ||
  fail "hexarchy-dns falls back to polkit where the sudoers grant is not installed" "got: $ungranted"

pass "hexarchy-dns falls back to polkit wherever the grant does not reach"
