...
cat >"$mock_bin/hexarchy-done" <<'SH'
#!/bin/bash
[[ $1 == "check" && $2 == "first-run-user" ]]
SH
cat >"$mock_bin/hexarchy-provision-user" <<'SH'
#!/bin/bash
touch "$HEXARCHY_TEST_FINALIZE_CALLED"
SH
chmod +x "$mock_bin/hexarchy-done" "$mock_bin/hexarchy-provision-user"

finalize_called="$test_tmp/finalize-called"
HOME="$test_tmp/home" PATH="$mock_bin:$PATH" HEXARCHY_TEST_FINALIZE_CALLED="$finalize_called" \
  bash "$ROOT/bin/hexarchy-provision-first-run" >"$test_tmp/output"
...
if grep -F 'user-migration-notify-watch-enabled' "$ROOT/bin/hexarchy-provision-first-run" >/dev/null; then
