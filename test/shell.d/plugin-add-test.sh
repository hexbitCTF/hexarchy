...
mkdir -p "$stub_dir"
cat >"$stub_dir/hexarchy-shell" <<'STUB'
#!/bin/bash
exit 0
STUB
chmod +x "$stub_dir/hexarchy-shell"

test_home="$TMPDIR/home"
write_plugin "$test_home/.config/hexarchy/plugins/different-folder" "acme.same" "Installed"
...
output=$(HOME="$test_home" HEXARCHY_PATH="$ROOT" PATH="$stub_dir:$ROOT/bin:$PATH" \
  hexarchy-plugin-add "$incoming" --yes 2>&1) &&
...
[[ ! -e $test_home/.config/hexarchy/plugins/acme.same ]] ||
...
# before `git clone` runs, matching hexarchy-theme-install. A git stub records
...
mkdir -p "$guard_stubs"
cat >"$guard_stubs/hexarchy-shell" <<'STUB'
#!/bin/bash
exit 0
STUB
chmod +x "$guard_stubs/hexarchy-shell"
...
add_url() {
  HOME="$test_home" HEXARCHY_PATH="$ROOT" PATH="$guard_stubs:$ROOT/bin:$PATH" \
    hexarchy-plugin-add "$1" --yes 2>&1
}

# Transport helpers reach the guard, are named as such, and never reach clone.
for bad in "ext::sh -c touch /tmp/hexarchy-guard-test" "fd::17"; do
  rm -f "$clone_marker"
  output=$(add_url "$bad") &&
...
for bad in "ext://sh -c id" "gcrypt://example.com/x"; do
  ...
  grep -qF "which Hexarchy does not clone from" <<<"$output" ||
...
if script -qec true /dev/null >/dev/null 2>&1; then
  rm -f "$clone_marker"
  status=0
  raw=$(GUM_INPUT_VALUE="-oProxyCommand=x" HOME="$test_home" HEXARCHY_PATH="$ROOT" \
    PATH="$guard_stubs:$ROOT/bin:$PATH" \
    script -qec "hexarchy-plugin-add --yes" /dev/null) || status=$?
  output=$(tr -d '\r' <<<"$raw")
  (( status != 0 )) ||
...
for good in \
  "https://github.com/acme/hexarchy-weather.git" \
