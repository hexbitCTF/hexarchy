...
mkdir -p "$TMPDIR/home/.config/hexarchy" "$TMPDIR/bin"
CALLS="$TMPDIR/calls"
...
if [[ $* == *"listShellConfig"* ]]; then
  ...
  if [[ ${FAKE_NO_DISCOVERY:-0} == 1 ]]; then
    ...
    find "$HOME/.config/hexarchy/plugins" -mindepth 2 -maxdepth 2 -name manifest.json -print0 |
      xargs -0 -r jq -s 'map({id: .id, enabled: true})'
  fi
elif [[ $* == *"setPluginEnabled"* ]]; then
  printf 'hexarchy-shell %s\n' "$*" >>"$FAKE_CALLS"
  printf 'ok\n'
fi
exit 0
SH

for command in hexarchy-plugin-enable hexarchy-notification-send fake-editor; do
...
clone_plugin() {
  ...
  HOME="$TMPDIR/home" USER=tester HEXARCHY_PATH="$ROOT" PATH="$TMPDIR/bin:$ROOT/bin:$PATH" \
    FAKE_CALLS="$CALLS" HEXARCHY_TEST_ROOT="$ROOT" \
    FAKE_SHELL_CONFIG="${FAKE_CLONE_CONFIG:-$default_config}" \
    hexarchy-plugin-clone "$@"
}

clone_plugin hexarchy.clock >/dev/null
clock="$TMPDIR/home/.config/hexarchy/plugins/tester.clock"
...
rg -qF "hexarchy.clock" "$clock" -g '*.qml' -g '*.js' ||
...
jq -e '
  ...
  .hexarchy.clonedFrom == "hexarchy.clock" and
...
grep -qx 'hexarchy-plugin-enable tester.clock' "$CALLS" ||
  fail "clone does not enable the editable copy"
grep -qx 'hexarchy-notification-send -g 󰐱 Editing Cloned Plugin Original plugin has been replace by clone.' "$CALLS" ||
...
clone_plugin hexarchy.keyboard-layout >/dev/null
grep -qx 'hexarchy-plugin-enable tester.keyboard-layout' "$CALLS" ||
...
clone_plugin hexarchy.menu >/dev/null
menu="$TMPDIR/home/.config/hexarchy/plugins/tester.menu"
...
grep -qx 'hexarchy-plugin-enable tester.menu' "$CALLS" ||
  fail "clone does not enable a multi-kind plugin"
pass "clone preserves and enables multi-kind plugins"

remove_output=$(HOME="$TMPDIR/home" HEXARCHY_PATH="$ROOT" PATH="$TMPDIR/bin:$ROOT/bin:$PATH" \
  FAKE_CALLS="$CALLS" HEXARCHY_TEST_ROOT="$ROOT" \
  hexarchy-plugin-remove tester.menu --yes)
grep -qx 'hexarchy-shell shell setPluginEnabled tester.menu false' "$CALLS" ||
  fail "removing an enabled clone does not disable it first"
grep -q 'Restored hexarchy.menu.' <<<"$remove_output" ||
...
clone_plugin hexarchy.active-window >/dev/null
[[ -f $TMPDIR/home/.config/hexarchy/plugins/tester.active-window/ActiveWindow.qml ]] ||
  fail "flat bar plugin clone is incomplete"
pass "flat bar plugins clone from adjacent manifests"

grep -qx 'hexarchy-plugin-enable tester.active-window' "$CALLS" ||
  fail "clone does not activate a bar widget whose source is absent"
pass "clone activates an absent bar widget"

clone_plugin hexarchy.indicators >/dev/null
indicators="$TMPDIR/home/.config/hexarchy/plugins/tester.indicators"
...
clone_plugin hexarchy.tray >/dev/null
[[ -f $TMPDIR/home/.config/hexarchy/plugins/tester.tray/TrayModel.js ]] ||
  fail "tray clone is missing its model"
pass "flat bar plugins keep local script dependencies"

clone_plugin hexarchy.bar >/dev/null
grep -qx 'hexarchy-plugin-enable tester.bar' "$CALLS" ||
  fail "clone does not select a cloned bar"
pass "clone switches full bars"

clone_plugin hexarchy.background >/dev/null
grep -qx 'hexarchy-plugin-enable tester.background' "$CALLS" ||
  fail "clone does not enable an ordinary cloned plugin"
pass "clone switches ordinary plugins"

EDITOR=fake-editor clone_plugin hexarchy.weather --edit >/dev/null
grep -qx "fake-editor $TMPDIR/home/.config/hexarchy/plugins/tester.weather" "$CALLS" ||
  fail "clone --edit does not open the clone in EDITOR"
pass "clone --edit opens the clone in EDITOR"
rm -rf "$TMPDIR/home/.config/hexarchy/plugins/tester.weather"

mkdir -p "$TMPDIR/home/.config/hexarchy/plugins/acme.example"
cat >"$TMPDIR/home/.config/hexarchy/plugins/acme.example/manifest.json" <<'JSON'
...
if clone_plugin hexarchy.weather custom.weather >/dev/null 2>&1; then
  fail "clone accepts a custom id"
fi
[[ ! -e $TMPDIR/home/.config/hexarchy/plugins/tester.weather ]] ||
  fail "rejected custom id leaves a clone behind"
pass "clone derives the personal id from the username"

if clone_plugin hexarchy.weather --replace >/dev/null 2>&1; then
...
if FAKE_NO_DISCOVERY=1 clone_plugin hexarchy.osd >/dev/null 2>&1; then
  fail "clone succeeds before the shell discovers it"
fi
[[ ! -e $TMPDIR/home/.config/hexarchy/plugins/tester.osd ]] ||
