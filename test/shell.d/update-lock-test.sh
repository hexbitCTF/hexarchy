...
for command in \
  ...
  hexarchy-shell \
  hexarchy-update-restart; do
  write_stub "$command" 'exit 0'
done
write_stub hexarchy-update-available 'exit 1'
write_stub pkexec 'exec "$@"'

# hexarchy-update should hold the lock before snapshotting, so a second update
# cannot even enter its pre-update snapshot.
update_snapshot_marker="$test_tmp/update-snapshot-started"
write_stub hexarchy-snapshot 'echo started >"$TEST_MARKER"; sleep 2; exit 0'

HEXARCHY_UPDATE_LOGGED=1 TEST_MARKER="$update_snapshot_marker" run_with_lock_env "$ROOT/bin/hexarchy-update" -y >"$test_tmp/update-first.out" 2>&1 &
update_pid=$!
...
[[ -f $update_snapshot_marker ]] || fail "first hexarchy-update reached snapshot under lock"

set +e
HEXARCHY_UPDATE_LOGGED=1 TEST_MARKER="$test_tmp/update-second-snapshot-started" run_with_lock_env "$ROOT/bin/hexarchy-update" -y >"$test_tmp/update-second.out" 2>&1
update_second_status=$?
set -e

wait "$update_pid"

[[ $update_second_status -ne 0 ]] || fail "second hexarchy-update exits non-zero while update lock is held"
grep -q "already running" "$test_tmp/update-second.out" || fail "second hexarchy-update reports held update lock"
[[ ! -f $test_tmp/update-second-snapshot-started ]] || fail "second hexarchy-update did not snapshot while lock was held"
pass "hexarchy-update prevents overlapping top-level updates"
...
# later update and silencing hexarchy-migrate-notify, which reads the same lock.
inhibit_pid_file="$test_tmp/inhibit-pid"
keyring_marker="$test_tmp/keyring-started"
write_stub hexarchy-snapshot 'exit 0'
write_stub systemd-inhibit 'echo "$$" >"$INHIBIT_PID_FILE"; exec sleep 30'
write_stub hexarchy-update-keyring 'echo started >"$TEST_MARKER"; sleep 3; exit 0'

HEXARCHY_UPDATE_LOGGED=1 TEST_MARKER="$keyring_marker" INHIBIT_PID_FILE="$inhibit_pid_file" \
  run_with_lock_env "$ROOT/bin/hexarchy-update" -y >"$test_tmp/update-inhibit.out" 2>&1 &
inhibit_update_pid=$!
...
lock_target=$(readlink -f "$runtime_dir/hexarchy-update.lock")
...
pass "hexarchy-update keeps the update lock out of its sleep inhibitor"

kill -0 "$inhibitor_pid" 2>/dev/null &&
  fail "update waits for its sleep inhibitor to stop before continuing"
pass "hexarchy-update waits for its sleep inhibitor to stop"
...
SH
  ...
  run_with_lock_env "$ROOT/bin/hexarchy-update-stay-awake" stop
...
write_stub hexarchy-snapshot 'exit 0'
write_stub hexarchy-update-keyring 'exit 0'
write_stub hexarchy-toggle-idle '
state_file="$HOME/.local/state/hexarchy/indicators/stay-awake"
case "$1" in
  stay-awake)
...
write_stub hexarchy-update-restart '
state_file="$HOME/.local/state/hexarchy/indicators/stay-awake"
...
rm -f "$test_home/.local/state/hexarchy/indicators/stay-awake"
HEXARCHY_UPDATE_LOGGED=1 run_with_lock_env "$ROOT/bin/hexarchy-update" -y
[[ ! -f $test_home/.local/state/hexarchy/indicators/stay-awake ]] || fail "update clears its Stay Awake state before restart handling"

mkdir -p "$test_home/.local/state/hexarchy/indicators"
touch "$test_home/.local/state/hexarchy/indicators/stay-awake"
HEXARCHY_UPDATE_LOGGED=1 EXPECT_STAY_AWAKE=1 run_with_lock_env "$ROOT/bin/hexarchy-update" -y
[[ -f $test_home/.local/state/hexarchy/indicators/stay-awake ]] || fail "update preserves pre-existing Stay Awake state"
pass "hexarchy-update restores only its own Stay Awake state before restart handling"

# Stale cleanup state from a killed update must not override a Stay Awake choice
# the user made afterward.
stay_awake_helper_state="$runtime_dir/hexarchy-update-stay-awake"
stay_awake_state="$test_home/.local/state/hexarchy/indicators/stay-awake"
