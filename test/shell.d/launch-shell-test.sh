...
# Each launch consumes the next status from HEXARCHY_TEST_QS_STATUSES; "run"
...
if [[ $status == "run" ]]; then
  trap 'touch "$HEXARCHY_TEST_QS_TERMINATED"; exit 143' TERM
...
[[ ${HEXARCHY_TEST_COMPOSITOR_GONE:-0} == 1 ]] && exit 4

# Refuse the first HEXARCHY_TEST_HYPRCTL_MISSES queries, then answer.
if (( ${HEXARCHY_TEST_HYPRCTL_MISSES:-0} > 0 )); then
  misses=$(cat "$HEXARCHY_TEST_HYPRCTL_MISS_COUNT" 2>/dev/null || printf '0')
  if (( misses < HEXARCHY_TEST_HYPRCTL_MISSES )); then
    printf '%s\n' "$(( misses + 1 ))" >"$HEXARCHY_TEST_HYPRCTL_MISS_COUNT"
    exit 4
  fi
fi
...
launch_shell() {
  ...
  PATH="$fake_bin:$PATH" \
  HEXARCHY_PATH="$shell_root" \
  HEXARCHY_TEST_QS_LOG="$qs_log" \
  HEXARCHY_TEST_QS_ENV_LOG="$qs_env_log" \
  HEXARCHY_TEST_QS_STATUSES="$1" \
  HEXARCHY_TEST_COMPOSITOR_GONE="${2:-0}" \
  HEXARCHY_TEST_LOGGER_LOG="$logger_log" \
  HEXARCHY_TEST_QS_TERMINATED="$qs_terminated" \
  HEXARCHY_TEST_HYPRCTL_MISSES="${3:-0}" \
  HEXARCHY_TEST_HYPRCTL_MISS_COUNT="$hyprctl_misses" \
    timeout 30 "$ROOT/bin/hexarchy-launch-shell"
}
...
grep -F -- "-n -p $shell_root/shell" "$qs_log" >/dev/null || fail "the shell launches from HEXARCHY_PATH"
...
HEXARCHY_TEST_QS_LOG="$qs_log" \
HEXARCHY_TEST_QS_ENV_LOG="$qs_env_log" \
HEXARCHY_TEST_QS_STATUSES=$'255\n0' \
HEXARCHY_TEST_COMPOSITOR_GONE=0 \
HEXARCHY_TEST_LOGGER_LOG="$logger_log" \
HEXARCHY_TEST_QS_TERMINATED="$qs_terminated" \
  "$ROOT/bin/hexarchy-launch-shell" &
launch_pid=$!
