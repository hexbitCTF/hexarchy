...
! grep -F 'systemd-networkd' "$dns" >/dev/null || fail "hexarchy-dns no longer restarts systemd-networkd"
grep -F 'NetworkManager/conf.d/20-hexarchy-dns.conf' "$dns" >/dev/null
...
if grep -F 'nmcli general reload conf,dns-full' "$dns" >/dev/null; then
  fail "hexarchy-dns must not push DNS before reapplying active profiles"
fi
pass "hexarchy-dns configures DNS through NetworkManager"
...
grep -F 'hexarchy-networkd-retired' "$hardware_network" >/dev/null
pass "hardware setup retires archinstall networkd state"

grep -F 'HEXARCHY_UPGRADE_TO_QUATTRO_LIVE' "$migration" >/dev/null
