...
manifest_path="$test_home/.config/chromium/NativeMessagingHosts/com.hexarchy.ytdlp.json"

HOME="$test_home" HEXARCHY_PATH="$ROOT" hexarchy-install-chromium-ytdlp
...
jq -e --arg path "$ROOT/bin/hexarchy-chromium-ytdlp-host" '
  .name == "com.hexarchy.ytdlp" and
  .path == $path and
  (.allowed_origins | index("chrome-extension://dedjgknigfeelejglamclffonmophnfl/"))
' "$manifest_path" >/dev/null
pass "yt-dlp native host manifest uses Hexarchy host path and extension id"

parse_result=$(bash -c '
  HEXARCHY_PATH="$3"
  source "$1"
  parse_url "$2"
' bash "$ROOT/bin/hexarchy-chromium-ytdlp-host" '{"url":"https://example.test/watch?v=\"quoted\"&name=a\\b"}' "$ROOT")
...
bash -c '
  ...
  source "$1"
  valid_url "$2"
' bash "$ROOT/bin/hexarchy-chromium-ytdlp-host" "javascript:alert(1)" "$ROOT" &&
  fail "yt-dlp native host rejects non-web URLs"
pass "yt-dlp native host rejects non-web URLs"

host_fn() {
  HEXARCHY_PATH="$ROOT" HEXARCHY_YTDLP_DIR="${download_dir:-$TMPDIR}" bash -c '
    source "$1"
    shift
    "$@"
  ' bash "$ROOT/bin/hexarchy-chromium-ytdlp-host" "$@"
}
...
forged_title=$(host_fn decode_title '"Clip\nHEXARCHY_FILE\tPlay me\t--include=not-a-file"')
[[ $forged_title == "Clip" ]] ||
...
while IFS= read -r line; do
  case $line in
  HEXARCHY_FILE*)
    resolved=$(resolve_download_file "${line#HEXARCHY_FILE$'\t'}") || continue
    filepath=$resolved
    ;;
  esac
done
...
poisoned=$(
  printf '%s\n' \
    $'HEXARCHY_FILE\t'"$good_file" \
    $'HEXARCHY_FILE\tPlay me\t--include=not-a-file' \
    $'\t'"$good_file" |
    HEXARCHY_PATH="$ROOT" HEXARCHY_YTDLP_DIR="$download_dir" bash "$parse_script" "$ROOT/bin/hexarchy-chromium-ytdlp-host"
)

[[ $poisoned == "$expected" ]] ||
  fail "yt-dlp native host keeps a real file after a forged HEXARCHY_FILE record" "$poisoned"
pass "yt-dlp native host keeps a real file after a forged HEXARCHY_FILE record"
...
for arg in "$@"; do
  if [[ $arg == "--no-simulate" ]]; then
    printf 'HEXARCHY_FILE\t%s\n' "$YTDLP_FAKE_FILE"
    [[ -n ${YTDLP_SKIP_TITLE:-} ]] || printf 'HEXARCHY_TITLE\t%s\n' '"My Great Clip"'
    exit 0
  fi
done
exit 0
...
cat >"$fake_root/bin/hexarchy-notification-send" <<'EOF'
#!/bin/bash
printf '%s\n' "$*" >>"$NOTIFY_ARGV_LOG"
EOF

for stub in hexarchy-osd hexarchy-shell ffmpeg; do
  printf '#!/bin/bash\nexit 0\n' >"$fake_root/bin/$stub"
done
chmod +x "$fake_root/bin/"*

YTDLP_ARGV_LOG="$ytdlp_argv" NOTIFY_ARGV_LOG="$notify_argv" YTDLP_FAKE_FILE="$fake_file" \
  HEXARCHY_PATH="$fake_root" HEXARCHY_YTDLP_DIR="$fake_dir" \
  bash -c '
    source "$1"
    download_url "$2"
  ' bash "$ROOT/bin/hexarchy-chromium-ytdlp-host" "https://example.test/watch" >/dev/null 2>&1
...
grep -qF -- $'HEXARCHY_TITLE\t%(title)j' "$ytdlp_argv" ||
...
grep -qF -- $'HEXARCHY_FILE\t%(title)s' "$ytdlp_argv" &&
...
: >"$ytdlp_argv"
YTDLP_ARGV_LOG="$ytdlp_argv" NOTIFY_ARGV_LOG="$notify_argv" YTDLP_FAKE_FILE="$fake_file" \
  YTDLP_SKIP_TITLE=1 HEXARCHY_PATH="$fake_root" HEXARCHY_YTDLP_DIR="$fake_dir" \
  bash -c '
    source "$1"
    download_url "$2"
  ' bash "$ROOT/bin/hexarchy-chromium-ytdlp-host" "https://example.test/watch" >/dev/null 2>&1
