...
#!/bin/bash
printf 'pkg:%s\n' "$*" >>"$HEXARCHY_TEST_LOG"
exit "${HEXARCHY_TEST_PKG_STATUS:-0}"
SH

for command in hexarchy-pkg-aur-add hexarchy-install-emacs omazed hexarchy-theme-set-vscode hexarchy-install-gaming-gpu-lib32; do
...
#!/bin/bash
printf '%s\n' "$1" >"$HEXARCHY_TEST_PRESENTATION"
SH

chmod +x "$mock_bin"/*

export HOME="$test_home"
export HEXARCHY_TEST_LOG="$test_tmp/launch.log"
export HEXARCHY_TEST_PRESENTATION="$test_tmp/presentation"
...
wait_for_launch() {
  ...
  for ((attempt = 0; attempt < 100; attempt++)); do
    grep -Fxq "$expected" "$HEXARCHY_TEST_LOG" && return 0
    sleep 0.01
  done
...
assert_detached_installer_launch hexarchy-install-editor-emacs emacsclient
assert_detached_installer_launch hexarchy-install-editor-vscode code
assert_detached_installer_launch hexarchy-install-editor-zed dev.zed.Zed
assert_detached_installer_launch hexarchy-install-gaming-heroic heroic
assert_detached_installer_launch hexarchy-install-gaming-steam steam

bash "$ROOT/bin/hexarchy-install-and-launch" "Example App" "alpha beta" "Disk Usage"
presentation_command=$(<"$HEXARCHY_TEST_PRESENTATION")
...
[[ $presentation_command == *'hexarchy-pkg-add alpha beta && (setsid uwsm-app -- gtk-launch Disk\ Usage >/dev/null 2>&1 &)'* ]] ||
...
grep -Fxq 'pkg:alpha beta' "$HEXARCHY_TEST_LOG" ||
...
: >"$HEXARCHY_TEST_LOG"
if HEXARCHY_TEST_PKG_STATUS=1 bash -c "$presentation_command"; then
  fail "generic installer propagates package installation failure"
fi
if grep -q '^launch:' "$HEXARCHY_TEST_LOG"; then
