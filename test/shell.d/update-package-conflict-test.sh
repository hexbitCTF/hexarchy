...
update_env() {
  printf '%s\n' \
    "HEXARCHY_REPLACED_DIR=$test_tmp/replaced" \
    "PACMAN_ATTEMPTS=$test_tmp/attempts" \
    "PACMAN_CALLS=$test_tmp/calls" \
    "CONFLICT_REPORT=$test_tmp/report" \
    "OWNED_PATHS=" \
    "HEXARCHY_UPDATE_UNATTENDED=${HEXARCHY_UPDATE_UNATTENDED:-}" \
    "HEXARCHY_UPDATE_INTERACTIVE=${HEXARCHY_UPDATE_INTERACTIVE:-}" \
...
run_headless() {
  ...
  env "${environment[@]}" bash "$ROOT/bin/hexarchy-update-system-pkgs" \
    </dev/null >"$test_tmp/out" 2>"$test_tmp/err"
}

# script gives the update the pty that hexarchy-update always runs it on, so the
...
run_on_terminal() {
  mapfile -t environment < <(update_env)
  env "${environment[@]}" \
    script -qec "bash '$ROOT/bin/hexarchy-update-system-pkgs' ${1:-}" "$test_tmp/out" >/dev/null 2>&1
}
...
grep -q 'hexarchy update' "$test_tmp/err" ||
...
write_conflict_report
if HEXARCHY_UPDATE_UNATTENDED=1 run_on_terminal; then
...
write_conflict_report
if HEXARCHY_UPDATE_INTERACTIVE=1 run_headless; then
