...
#!/bin/bash
printf '%s:%s:%s\\n' '$prefix' "\${HEXARCHY_REMOVE_NOTIFY:-}" "\$*" >>"\$TEST_LOG"
SCRIPT
  chmod +x "$tmp_dir/bin/$name"
}

write_fake_command hexarchy-webapp-remove web
write_fake_command hexarchy-tui-remove tui
write_fake_command hexarchy-launch-floating-terminal-with-presentation terminal

cat >"$tmp_dir/bin/hexarchy-notification-send" <<'SCRIPT'
#!/bin/bash
printf 'notify::%s\n' "$*" >>"$TEST_LOG"
SCRIPT
chmod +x "$tmp_dir/bin/hexarchy-notification-send"
...
[Desktop Entry]
Name=Basecamp
Exec=hexarchy-launch-webapp https://example.com
DESKTOP
...
"$ROOT/bin/hexarchy-remove-launcher-entry" Basecamp.desktop Basecamp
"$ROOT/bin/hexarchy-remove-launcher-entry" Docker.desktop Docker
"$ROOT/bin/hexarchy-remove-launcher-entry" native.desktop Native
"$ROOT/bin/hexarchy-remove-launcher-entry" aliens.desktop Aliens
