...
run_dev_update() {
  HEXARCHY_PATH="$1" \
    ...
    PATH="$stub_bin:$PATH" \
    "$ROOT/bin/hexarchy-update-dev"
}

: >"$git_log"
run_dev_update /usr/share/hexarchy
...
grep -F "HEXARCHY_PATH is not a git checkout: $checkout" "$test_tmp/invalid.err" >/dev/null ||
...
grep -qE '^ *hexarchy-update-dev$' "$ROOT/bin/hexarchy-update" ||
