...
output=$(hexarchy-ascii Hexarchy)
...
output=$(printf 'Hexarchy' | hexarchy-ascii)
...
# through `hexarchy` itself, not just through the binary.
output=$(printf 'Hexarchy' | hexarchy ascii)
[[ $output == "$expected" ]] || fail "piped text renders through the hexarchy route" "got:
$output"
pass "piped text renders through the hexarchy route"
...
tight=$(hexarchy-ascii "Hi" | columns)
spaced=$(hexarchy-ascii "H i" | columns)
...
status=0
warning=$(hexarchy-ascii "Hexarchy 4.0" 2>&1 >/dev/null) || status=$?
...
output=$(hexarchy-ascii "Hexarchy 4.0" 2>/dev/null)
...
status=0
output=$(hexarchy-ascii "4.0" 2>/dev/null) || status=$?
...
status=0
output=$(hexarchy-ascii --width 40 2>&1 >/dev/null) || status=$?
...
output=$(hexarchy-ascii -- Hi | columns)
(( output == 18 )) || fail "text after -- is still text" "got $output columns"
pass "text after -- is still text"

output=$(hexarchy-ascii --help)
[[ $output == *"Usage: hexarchy-ascii"* ]] || fail "help renders"
pass "help renders"
