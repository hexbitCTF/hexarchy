...
grep -q "sendNativeMessage('com.hexarchy.copy_url'" \
...
native_manifest="$test_home/.config/chromium/NativeMessagingHosts/com.hexarchy.copy_url.json"

HOME="$test_home" HEXARCHY_PATH="$ROOT" hexarchy-install-chromium-copy-url

[[ -f $native_manifest ]] || fail "copy-url native host installer creates fresh Chromium profile root"
jq -e --arg path "$ROOT/bin/hexarchy-chromium-copy-url-host" '
  .name == "com.hexarchy.copy_url" and
  .path == $path and
  (.allowed_origins | index("chrome-extension://bgpiichlckmfanooecilcjemknkcpngb/"))
' "$native_manifest" >/dev/null || fail "copy-url native host manifest uses Hexarchy host path and extension id"
...
# hexarchy-install-browser, and they mark every migration as already applied.
...
HOME="$fresh_home" HEXARCHY_PATH="$ROOT" PATH="$ROOT/bin:$PATH" \
  bash -euo pipefail -c 'source "$ROOT/install/user/chromium.sh"'

[[ -f $fresh_home/.config/chromium/NativeMessagingHosts/com.hexarchy.copy_url.json ]] ||
...
copied_url=$(bash -c '
  ...
  copy_url "$2"
' bash "$ROOT/bin/hexarchy-chromium-copy-url-host" 'https://example.test/path?q=one&name=two')
...
native_reply=$(bash -c '
  source "$1"
  reply_copied true
' bash "$ROOT/bin/hexarchy-chromium-copy-url-host" | od -An -v -tx1 | tr -d ' \n')
