...
other_packages="$ROOT/install/hexarchy-other.packages"
...
cat >"$stub_bin/hexarchy-pkg-present" <<'SH'
#!/bin/bash
...
printf 'hexarchy-pkg-drop' >>"$TEST_LOG"
printf '\t%s' "$@" >>"$TEST_LOG"
printf '\n' >>"$TEST_LOG"
SH
...
cat >"$limine_conf" <<'EOF'
# Generated by Hexarchy installer for T2 Mac support
...
PATH="$stub_bin:$PATH" \
  ...
  HEXARCHY_T2_LIMINE_CONF="$limine_conf" \
  HEXARCHY_T2_FAN_CONF="$fan_conf" \
  HEXARCHY_T2_RUNNING_CMDLINE="$running_cmdline" \
  HEXARCHY_T2_REPAIR_MARKER="$repair_marker" \
...
grep -Fq $'hexarchy-pkg-drop\ttiny-dfr' "$calls" ||
...
! grep -Eq $'^(sudo\t)?(sed|tee|systemctl|hexarchy-pkg-drop)(\t|$)' "$calls" ||
