...
cat >"$stub_bin/hexarchy-shell" <<'SH'
#!/bin/bash

printf '%s\n' "$*" >>"$SHELL_CALLS"
SH

chmod +x "$stub_bin/hexarchy-update-available" "$stub_bin/hexarchy-shell"

PATH="$stub_bin:$PATH" SHELL_CALLS="$shell_calls" UPDATE_AVAILABLE_STATUS=0 \
  "$ROOT/bin/hexarchy-update-status"
grep -Fx -- "-q hexarchy.system-update refresh" "$shell_calls" >/dev/null ||
...
PATH="$stub_bin:$PATH" SHELL_CALLS="$shell_calls" UPDATE_AVAILABLE_STATUS=1 \
  "$ROOT/bin/hexarchy-update-status"
grep -Fx -- "-q hexarchy.system-update clear" "$shell_calls" >/dev/null ||
