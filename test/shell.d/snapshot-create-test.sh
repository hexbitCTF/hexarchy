...
snapshot="$ROOT/bin/hexarchy-snapshot"
...
cat >"$fake_bin/hexarchy-cmd-missing" <<'STUB'
#!/bin/bash
exit 1
STUB
chmod +x "$fake_bin/hexarchy-cmd-missing"

cat >"$fake_bin/hexarchy-version" <<'STUB'
#!/bin/bash
echo 4.0.0
STUB
chmod +x "$fake_bin/hexarchy-version"
...
grep -qF 'hexarchy-snapshot create || (($? == 127))' "$ROOT/bin/hexarchy-update" ||
...
grep -qF 'hexarchy-snapshot create || (($? == 127))' "$ROOT/bin/hexarchy-upgrade-to-quattro" ||
  fail "upgrade ignores only the missing-snapper exit code"
grep -qF 'Continuing the upgrade without a snapshot' "$ROOT/bin/hexarchy-upgrade-to-quattro" ||
