...
for (const manifestPath of manifests) {
  ...
  check(String(manifest.id).startsWith('hexarchy.'), `${manifest.id} must use the first-party namespace`)
  ...
  const clonePaths = manifest.hexarchy?.clonePaths
  if (clonePaths !== undefined) {
    check(Array.isArray(clonePaths), `${manifest.id} hexarchy.clonePaths must be an array`)
...
for (const [id, section] of Object.entries({
  'hexarchy.active-window': 'left',
  'hexarchy.dropbox': 'right'
})) {
  check(byId[id]?.barWidget?.defaultSection === section, `${id} must default to the ${section} bar section`)
}
check(byId['hexarchy.media']?.barWidget?.defaultSection === undefined, 'hexarchy.media must use the center fallback')
