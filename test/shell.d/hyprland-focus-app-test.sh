...
#!/bin/bash
if [[ $1 == "clients" ]]; then
  printf '%s\n' "$HEXARCHY_TEST_CLIENTS_JSON"
elif [[ $1 == "dispatch" ]]; then
  printf '%s\n' "$2" >"$HEXARCHY_TEST_FOCUS_DISPATCH"
fi
SH
...
PATH="$mock_bin:$PATH" HEXARCHY_TEST_CLIENTS_JSON="$clients_json" \
  HEXARCHY_TEST_FOCUS_DISPATCH="$dispatch_log" \
  bash "$ROOT/bin/hexarchy-hyprland-focus-app" '^chromium$'
...
clients_json='[
  ...
  {"address":"0xagent","class":"org.hexarchy.agent","initialClass":"org.hexarchy.agent","initialTitle":"kitty"}
]'
PATH="$mock_bin:$PATH" HEXARCHY_TEST_CLIENTS_JSON="$clients_json" \
  HEXARCHY_TEST_FOCUS_DISPATCH="$dispatch_log" \
  bash "$ROOT/bin/hexarchy-hyprland-focus-app" kitty
...
rm -f "$dispatch_log"
if PATH="$mock_bin:$PATH" HEXARCHY_TEST_CLIENTS_JSON="$clients_json" \
  HEXARCHY_TEST_FOCUS_DISPATCH="$dispatch_log" \
  bash "$ROOT/bin/hexarchy-hyprland-focus-app" Mail; then
