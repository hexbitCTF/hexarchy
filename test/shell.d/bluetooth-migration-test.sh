...
cat >"$test_dir/bin/hexarchy-bluetooth-power" <<'STUB'
#!/bin/bash

printf 'hexarchy-bluetooth-power %s\n' "$*" >>"$CALLS"
[[ $1 == "is-on" ]] || exit 0
[[ ${POWERED:-} == "yes" ]]
STUB
...
run_migration() {
  ...
  HEXARCHY_BLUETOOTH_MIGRATION_MARKER="$marker" \
    HEXARCHY_BLUETOOTH_MAIN_CONF="$main_conf" \
...
grep -qx 'hexarchy-bluetooth-power on' "$CALLS" ||
...
grep -qx 'hexarchy-bluetooth-power off' "$CALLS" ||
...
grep -qx 'sudo hexarchy-bluetooth-power off' "$CALLS" ||
...
# Only the exact line Hexarchy wrote is reverted, so a hand-edited opt-out stands.
reset_machine
printf '[Policy]\nAutoEnable = false\n' >"$main_conf"
POWERED=yes run_migration
