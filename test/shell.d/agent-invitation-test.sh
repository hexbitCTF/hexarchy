...
test_bin=$(mktemp -d)
log_file=$(mktemp)
hook_path="$test_home/.config/hexarchy/hooks/post-update.d/setup-agent.hook"
...
[[ -f $test_home/.local/state/hexarchy/done/agent-setup-invitation ]] || fail "agent invitation records completion"
...
grep -qx 'exec:hexarchy menu summon setup.default.agent' "$log_file" ||
...
fresh_home=$(mktemp -d)
mkdir -p "$fresh_home/.config/hexarchy/defaults"
printf 'claude\n' >"$fresh_home/.config/hexarchy/defaults/agent"
: >"$log_file"
cp "$ROOT/install/user/first-run/setup-agent.hook" "$hook_path"
...
[[ ! -f $fresh_home/.local/state/hexarchy/done/agent-setup-invitation ]] ||
