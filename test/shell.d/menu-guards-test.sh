...
const items = {
  'setup.default.browser.brave': { id: 'setup.default.browser.brave', when: 'hexarchy-pkg-present brave-bin', checked: '[[ "$(hexarchy-default-browser)" == "brave" ]]' },
  'setup.default.browser.zen': { id: 'setup.default.browser.zen', when: 'hexarchy-pkg-present zen-browser-bin', checked: '[[ "$(hexarchy-default-browser)" == "zen" ]]' },
  'install.browser.zen': { id: 'install.browser.zen', disabled: 'hexarchy-pkg-present zen-browser-bin' },
  'plain': { id: 'plain', label: 'No guards' }
}
const script = menu.guardScript(items)
const browserSlot = `\${__hexarchy_read_${menu.guardReaders.indexOf('hexarchy-default-browser')}}`

assert(
  script.includes('if { hexarchy-pkg-present brave-bin; } >/dev/null 2>&1; then echo setup.default.browser.brave:w:1; else echo setup.default.browser.brave:w:0; fi'),
...
assert(
  script.includes('if { hexarchy-pkg-present zen-browser-bin; } >/dev/null 2>&1; then echo install.browser.zen:d:1; else echo install.browser.zen:d:0; fi'),
...
assertEqual(
  (script.match(/^__hexarchy_read_\d+=\$\(hexarchy-default-browser /gm) || []).length,
  1,
  'guard script reads a value command once for the whole batch'
)
assert(
  script.includes(`[[ "${browserSlot}" == "brave" ]]`) && !script.includes('"$(hexarchy-default-browser)"'),
  'guard script substitutes the captured answer into the expression'
)
assert(
  script.indexOf('__hexarchy_read_') < script.indexOf('if { hexarchy-pkg-present'),
...
const untouched = menu.guardScript({
  a: { id: 'a', when: 'command -v hexarchy-dns' },
  b: { id: 'b', when: '[[ "$(HEXARCHY_PATH=/usr/share/hexarchy hexarchy-channel-current)" == "stable" ]]' },
  c: { id: 'c', when: '(( $(hexarchy-default-browser | wc -l) == 1 ))' }
})
assert(
  untouched.includes('command -v hexarchy-dns')
    && untouched.includes('$(HEXARCHY_PATH=/usr/share/hexarchy hexarchy-channel-current)')
    && untouched.includes('$(hexarchy-default-browser | wc -l)'),
  'guard script leaves every form but the plain substitution to run the real command'
)
assert(
  !/^__hexarchy_read_/m.test(untouched),
...
const fs = require('fs')
const defaultItems = menu.parseMenuJsonc(fs.readFileSync(path.join(root, 'default/hexarchy/hexarchy-menu.jsonc'), 'utf8'))
const guardText = defaultItems.map(item => `${item.when}\n${item.checked}\n${item.disabled}`).join('\n')
const repeated = [...new Set(
  (guardText.match(/\$\((hexarchy-[a-z0-9-]+)\)/g) || []).map(match => match.slice(2, -1))
...
for helper in hexarchy-pkg-present hexarchy-pkg-missing; do
...
for helper in hexarchy-cmd-present hexarchy-cmd-missing; do
  for case in "${cmd_cases[@]}"; do
    read -r -a argv <<<"$case"
    assert_helper_agrees "guard prelude resolves commands as the real helper does" "$helper" "${argv[@]}"
  done
done
pass "guard prelude resolves commands as hexarchy-cmd-present and hexarchy-cmd-missing do"
...
reader_script=$(node -e '
  ...
  process.stdout.write(menu.guardScript({
    hit: { id: "hit", checked: "[[ \"$(hexarchy-dns)\" == \"Cloudflare\" ]]" },
    miss: { id: "miss", checked: "[[ \"$(hexarchy-dns)\" == \"Google\" ]]" }
  }))
')
reader_result=$(bash -c '
hexarchy-dns() { printf "Cloudflare\n"; }
export -f hexarchy-dns
'"$reader_script")
...
# Update > Extra Themes runs hexarchy-theme-update, which pulls the themes that
...
# hexarchy-theme-extras today; the shapes below are what would tell us if one
# of them stopped.
themes_guard=$(node -e '
  const fs = require("fs")
  const path = require("path")
  const menu = require(path.join(process.env.ROOT, "shell/plugins/menu/MenuModel.js"))
  const items = menu.parseMenuJsonc(fs.readFileSync(path.join(process.env.ROOT, "default/hexarchy/hexarchy-menu.jsonc"), "utf8"))
...
assert_themes_guard_agrees() {
  ...
  [[ -n $(HOME="$home" PATH="$ROOT/bin:$stub_dir:$PATH" "$ROOT/bin/hexarchy-theme-update" 2>/dev/null) ]] || updated=1
...
mkdir -p "$themes_home/empty/.config/hexarchy/themes"
mkdir -p "$themes_home/copied/.config/hexarchy/themes/handmade"
mkdir -p "$themes_home/cloned/.config/hexarchy/themes/tokyo-night/.git"
mkdir -p "$themes_home/linked/.config/hexarchy/themes" "$themes_home/checkout/.git"
ln -s "$themes_home/checkout" "$themes_home/linked/.config/hexarchy/themes/in-progress"
mkdir -p "$themes_home/worktree/.config/hexarchy/themes/branch"
printf 'gitdir: /elsewhere\n' >"$themes_home/worktree/.config/hexarchy/themes/branch/.git"

for shape in missing:1 empty:1 copied:1 cloned:0 linked:1 worktree:1; do
  assert_themes_guard_agrees \
    "Extra Themes shows exactly when hexarchy-theme-update has something to pull" \
    "$themes_home/${shape%:*}" "${shape#*:}"
done
pass "Extra Themes shows exactly when hexarchy-theme-update has something to pull"
...
# directory.
many="$themes_home/many/.config/hexarchy/themes"
mkdir -p "$many/tokyo night/.git" "$many/zen/.git" "$many/handmade"
ln -s "$themes_home/checkout" "$many/in-progress"

listed=$(HOME="$themes_home/many" LC_ALL=C "$ROOT/bin/hexarchy-theme-extras")
[[ $listed == "$many/tokyo night"$'\n'"$many/zen" ]] ||
  fail "hexarchy-theme-extras lists every clone and nothing else" "got: $listed"
pass "hexarchy-theme-extras lists every clone and nothing else"
...
HOME="$themes_home/many" LC_ALL=C GIT_CALLS="$git_calls" PATH="$ROOT/bin:$stub_dir:$PATH" \
  "$ROOT/bin/hexarchy-theme-update" >/dev/null 2>&1
pulled=$(<"$git_calls")
[[ $pulled == "<-C><$many/tokyo night><pull>"$'\n'"<-C><$many/zen><pull>" ]] ||
  fail "hexarchy-theme-update pulls each clone by its whole path" "got: $pulled"
pass "hexarchy-theme-update pulls each clone by its whole path"
