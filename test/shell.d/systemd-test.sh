...
sleep_service="$ROOT/default/systemd/user/hexarchy-sleep-lock.service"
grep -Fx 'ExecStart=/usr/bin/hexarchy-system-sleep-monitor' "$sleep_service" >/dev/null
...
grep -Fx 'ConditionEnvironment=HEXARCHY_PATH' "$sleep_service" >/dev/null ||
  fail "sleep lock monitor can start without the Hexarchy shell path"
...
grep -F 'hexarchy-sleep-lock.service' "$first_run_units" >/dev/null
...
grep -F 'ExecStart=%h/.local/share/hexarchy/bin/hexarchy-system-sleep-monitor' "$upgrade_to_quattro" >/dev/null
grep -F 'ExecStart=/usr/bin/hexarchy-system-sleep-monitor' "$upgrade_to_quattro" >/dev/null
grep -F 'reset-failed hexarchy-sleep-lock.service' "$upgrade_to_quattro" >/dev/null
pass "Hexarchy 4 upgrade repairs the legacy sleep lock unit path"

[[ -e $ROOT/default/systemd/user/hexarchy-update-user-notify.path ]] &&
  fail "the retired migration watcher is back; pacman writing the migration directory during hexarchy update would notify about migrations that update is already applying"
grep -rlE '^(Path[A-Za-z]+|DirectoryNotEmpty)=.*/usr/share/hexarchy/migrations' "$ROOT/default/systemd/user" >/dev/null 2>&1 &&
...
notify_service="$ROOT/default/systemd/user/hexarchy-migrate-notify.service"
grep -Fx 'ExecStart=/usr/bin/hexarchy-migrate-notify' "$notify_service" >/dev/null
...
grep -F 'hexarchy-migrate-notify.service' "$first_run_units" >/dev/null ||
  fail "first-run does not enable the login migration notifier"
grep -F 'hexarchy-update-user-notify' "$first_run_units" >/dev/null &&
  fail "first-run still enables the retired notifier units"
pass "first-run enables the login-only migration notifier"

fcitx_service="$ROOT/default/systemd/user/hexarchy-fcitx5.service"
...
oomd_conf="$ROOT/etc/systemd/oomd.conf.d/10-hexarchy.conf"
