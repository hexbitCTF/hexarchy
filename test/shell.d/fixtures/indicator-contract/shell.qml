...
ShellRoot {
  ...
  property string resultPath: Quickshell.env("HEXARCHY_QML_TEST_RESULT")
  ...
  QtObject {
    ...
    function firstPartyServiceFor(id) {
      if (id === "hexarchy.notifications") return notificationService
      if (id === "hexarchy.idle") return idleService
      if (id === "hexarchy.nightlight") return nightlightService
      return null
    }
  }
  ...
  Timer {
    ...
    onTriggered: {
      ...
      if (screenRecording) {
        ...
        root.assertTrue(root.commandCount("hexarchy-menu toggle trigger.capture.screenrecord") === 1, "Screen Recording left click opens capture menu when idle")
        screenRecording.recording = true
        screenRecording.triggerPress(Qt.LeftButton)
        root.assertTrue(root.commandCount("hexarchy-capture-screenrecording --stop-recording") === 1, "Screen Recording left click stops active recording")
      }
      ...
      if (dictation) {
        ...
        root.assertTrue(root.commandCount("hexarchy-voxtype-config") === 2, "Dictation clicks run config command")
        root.assertTrue(root.commandCount("hexarchy-voxtype-model") === 0, "Dictation clicks do not run model command")
      }
