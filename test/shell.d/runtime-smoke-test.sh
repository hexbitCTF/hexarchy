...
shell_ipc() {
  HEXARCHY_PATH="$test_root" "$ROOT/bin/hexarchy-shell" "$@"
}

shell_ipc_quiet() {
  HEXARCHY_PATH="$test_root" "$ROOT/bin/hexarchy-shell" -q "$@"
}
...
# Every plugin under ~/.config/hexarchy/plugins hot-reloads, whoever wrote it.
hot_reload_id="acme.hot-reload"
hot_reload_dir="$test_home/.config/hexarchy/plugins/$hot_reload_id"
...
cat >"$hot_reload_dir/manifest.json" <<JSON
{
  ...
  "hexarchy": {"clonedFrom": "hexarchy.emojis"}
}
JSON
...
cat >"$stub_bin/hexarchy-update-available" <<'SH'
#!/bin/bash
echo "Hexarchy update available (test)"
exit 0
SH
chmod +x "$stub_bin/hexarchy-update-available"
...
jq -e '
  map(.id) as $ids |
  all(["hexarchy.menu", "hexarchy.notifications", "hexarchy.clock", "hexarchy.osd"][]; $ids | index(.)) and
...
[[ $(shell_ipc shell summon hexarchy.emojis "{}") == "ok" ]] ||
  fail_with_log "calls to a cloned source id do not reach its enabled clone"
shell_ipc_quiet shell hide hexarchy.emojis >/dev/null
...
[[ $(shell_ipc shell summon hexarchy.menu '{"menu":"apps"}') == "ok" ]] || fail_with_log "shell IPC summons menu apps overlay"
shell_ipc_quiet shell hide hexarchy.menu >/dev/null
...
shell_ipc_quiet hexarchy.system-update refresh >/dev/null 2>&1 || true
sleep 0.8

default_ids=$(jq -c '(.bar.layout.left + .bar.layout.center + .bar.layout.right) | map(.id // .)' "$ROOT/config/hexarchy/shell.json")
visible_default_ids='[
  "hexarchy.menu",
  "hexarchy.workspaces",
  "hexarchy.clock",
  "hexarchy.weather",
  "hexarchy.system-update",
  "hexarchy.network",
  "hexarchy.audio",
  "hexarchy.monitor"
]'
...
jq -e '
  ...
  ($center | index("hexarchy.weather")) != null and
  ($center | index("hexarchy.system-update")) != null and
  ($center | index("hexarchy.indicators")) != null and
  (($center | index("hexarchy.weather")) < ($center | index("hexarchy.system-update"))) and
  (($center | index("hexarchy.system-update")) < ($center | index("hexarchy.indicators")))
' <<<"$geometry" >/dev/null || {
  printf 'Geometry:\n' >&2
  jq . <<<"$geometry" >&2
...
for panel_id in hexarchy.audio hexarchy.bluetooth hexarchy.monitor hexarchy.network hexarchy.power; do
...
HOME="$test_home" HEXARCHY_PATH="$test_root" PATH="$ROOT/bin:$PATH" "$ROOT/bin/hexarchy-plugin-disable" hexarchy.audio

for _ in {1..80}; do
  shell_config=$(shell_ipc shell listShellConfig 2>/dev/null || true)
  geometry=$(shell_ipc shell debugBarGeometry 2>/dev/null || true)
  if jq -e 'all(.bar.layout.right[]; (.id // .) != "hexarchy.audio")' <<<"$shell_config" >/dev/null 2>&1 && \
     jq -e 'all(.[]; .id != "hexarchy.audio")' <<<"$geometry" >/dev/null 2>&1; then
    break
  fi
  if ! kill -0 "$QS_PID" 2>/dev/null; then
...
jq -e 'all(.bar.layout.right[]; (.id // .) != "hexarchy.audio")' <<<"$shell_config" >/dev/null || {
  printf 'Shell config after reload:\n%s\n' "$shell_config" | jq . >&2
  fail_with_log "plugin disable reloads shell config"
}

jq -e 'all(.[]; .id != "hexarchy.audio")' <<<"$geometry" >/dev/null || {
...
bar_put() {
  HOME="$test_home" HEXARCHY_PATH="$test_root" PATH="$ROOT/bin:$PATH" "$ROOT/bin/hexarchy-bar" put "$@"
}
...
bar_put hexarchy.keyboard-layout --after hexarchy.clock >/dev/null
for _ in {1..80}; do
  [[ $(center_ids) == *hexarchy.keyboard-layout* ]] && break
...
jq -e '
  ...
  | ($ids | index("hexarchy.clock")) as $clock
  | ($ids | index("hexarchy.keyboard-layout")) as $widget
...
placed=$(center_ids)
bar_put hexarchy.keyboard-layout --section right >/dev/null
sleep 0.5
[[ $(center_ids) == "$placed" ]] ||
  fail_with_log "bar put left a widget already on the bar alone (was $placed, now $(center_ids))"
jq -e 'all(.bar.layout.right[]; (.id // .) != "hexarchy.keyboard-layout")' \
  <<<"$(shell_ipc shell listShellConfig)" >/dev/null ||
