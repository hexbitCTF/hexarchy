...
sudoers_file="$test_tmp/hexarchy-dev-path"
mkdir -p "$stub_bin" "$test_tmp/home"

cat >"$stub_bin/sudo" <<'SH'
#!/bin/bash

printf 'sudo' >>"$HEXARCHY_DEV_LINK_TEST_LOG"
for arg in "$@"; do
  printf '\t%s' "$arg" >>"$HEXARCHY_DEV_LINK_TEST_LOG"
done
printf '\n' >>"$HEXARCHY_DEV_LINK_TEST_LOG"

case "$1" in
  tee)
    cat >"$HEXARCHY_DEV_LINK_TEST_CONF"
    ;;
  install)
    # The staged file is the second-to-last argument.
    cp "${@: -2:1}" "$HEXARCHY_DEV_LINK_TEST_SUDOERS"
    ;;
esac
SH
...
printf 'gum' >>"$HEXARCHY_DEV_LINK_TEST_LOG"
for arg in "$@"; do
  printf '\t%s' "$arg" >>"$HEXARCHY_DEV_LINK_TEST_LOG"
done
printf '\n' >>"$HEXARCHY_DEV_LINK_TEST_LOG"
...
run_link() {
  HOME="$test_tmp/home" \
    HEXARCHY_DEV_LINK_TEST_LOG="$log_file" \
    HEXARCHY_DEV_LINK_TEST_CONF="$conf_file" \
    HEXARCHY_DEV_LINK_TEST_SUDOERS="$sudoers_file" \
    PATH="$stub_bin:$PATH" \
    "$ROOT/bin/hexarchy-dev-link" "$@"
}
...
[[ $(<"$conf_file") == "export HEXARCHY_PATH=\"$checkout\"" ]] ||
  fail "dev link points HEXARCHY_PATH at the checkout" "$(<"$conf_file")"
pass "dev link points HEXARCHY_PATH at the checkout"

# sudo reads secure_path, not the caller's PATH, so the checkout has to come
# first there too or `sudo hexarchy-*` runs the packaged copy.
...
grep -Eq $'^sudo\tinstall\t-Dm440\t-o\troot\t-g\troot\t[^\t]+\t/etc/sudoers\\.d/hexarchy-dev-path$' "$log_file" ||
...
grep -F "sudo now resolves hexarchy-* from $checkout/bin" "$test_tmp/link.out" >/dev/null ||
