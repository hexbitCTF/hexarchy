...
sleep_monitor="$ROOT/bin/hexarchy-system-sleep-monitor"
tmpdir=$(mktemp -d)
trap 'rm -rf "$tmpdir"' EXIT

mock_bin="$tmpdir/bin"
mock_hexarchy="$tmpdir/hexarchy"
producer_pid_file="$tmpdir/producer-pid"
lock_log="$tmpdir/lock-log"
mkdir -p "$mock_bin" "$mock_hexarchy/bin"
...
cat >"$mock_hexarchy/bin/hexarchy-system-sleep-lock" <<'SH'
#!/bin/bash
...
chmod +x \
  ...
  "$mock_hexarchy/bin/hexarchy-system-sleep-lock"
ln -s "$sleep_monitor" "$mock_hexarchy/bin/hexarchy-system-sleep-monitor"

start_us=${EPOCHREALTIME//[!0-9]/}
HEXARCHY_PATH="$mock_hexarchy" \
  PATH="$mock_bin:$PATH" \
