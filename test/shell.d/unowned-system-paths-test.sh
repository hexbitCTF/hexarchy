...
# A file Hexarchy writes into /usr belongs to nobody, and the
# day a package starts shipping that same path, pacman refuses the upgrade for
# everyone who has the file. hexarchy-update-system-pkgs-when-conflicted recovers from
...
# Paths Hexarchy writes into /usr that no package owns, with the reason.
allowed = {
  ...
  # than Hexarchy's, which is why neither is a candidate for hexarchy-settings.
  "/usr/share/chromium/extensions",
  "/usr/lib/firefox/distribution",
  # Static content that belongs in hexarchy-settings. It cannot move there in the
  # same release that first ships hexarchy-update-system-pkgs-when-conflicted: the
...
if pkgs_root is None:
  print("not ok - hexarchy-pkgs checkout found for package ownership check", file=sys.stderr)
  sys.exit(1)

packaged = "\n".join(p.read_text() for p in pkgs_root.glob("*/PKGBUILD"))

# Commands that put a file somewhere, as opposed to reading one.
# /etc is administrator territory that Hexarchy legitimately edits. /usr is
    ...
    for lineno, line in joined:
      ...
      if redirect:
        ...
        if not tokens or not tokens[-1].startswith("/usr/"):
          continue
        hit = tokens[-1].rstrip("/")
      # Hexarchy's own tree and its binaries are covered elsewhere.
      if hit.startswith(("/usr/share/hexarchy", "/usr/bin")) or hit.count("/") < 3:
        continue
...
if problems:
  print("not ok - Hexarchy writes paths under /usr that no package owns", file=sys.stderr)
  for p in problems:
...
pass "no Hexarchy script writes a path under /usr that no package owns"
