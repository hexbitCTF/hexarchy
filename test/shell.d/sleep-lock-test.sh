...
sleep_lock="$ROOT/bin/hexarchy-system-sleep-lock"
tmpdir=$(mktemp -d)
...
printf '%s\n' "\$*" >>"$notify_log"
SH
  chmod +x "$mock_bin/hexarchy-notification-send"
}
...
mock_clamshell() {
  cat >"$mock_bin/hexarchy-hyprland-monitor-clamshell" <<SH
#!/bin/bash

echo clamshell >>"\$CALL_LOG"
sleep ${1:-0}
SH
  chmod +x "$mock_bin/hexarchy-hyprland-monitor-clamshell"
}
...
SH
chmod +x "$mock_bin/hexarchy-shell"
mock_clamshell 2
