...
FAKE_HYPRCTL_LOG="$hyprctl_log" \
  ...
  HEXARCHY_HYPRLAND_RELOAD_GUARD_RUN_ROOT="$run_root" \
  HEXARCHY_HYPRLAND_RELOAD_GUARD_STATE_DIR="$state_dir" \
  "$ROOT/bin/hexarchy-hyprland-reload-guard" pause 2>"$test_tmp/pause-stderr"
...
FAKE_HYPRCTL_LOG="$hyprctl_log" \
  ...
  HEXARCHY_HYPRLAND_RELOAD_GUARD_RUN_ROOT="$run_root" \
  HEXARCHY_HYPRLAND_RELOAD_GUARD_STATE_DIR="$state_dir" \
  "$ROOT/bin/hexarchy-hyprland-reload-guard" resume
...
rm -rf "$state_dir"
HEXARCHY_HYPRLAND_RELOAD_GUARD_STATE_DIR="$state_dir" "$ROOT/bin/hexarchy-hyprland-reload-guard" paused &&
