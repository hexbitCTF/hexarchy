...
test_bin=$(mktemp -d)
log_file=$(mktemp)
hook_path="$test_home/.config/hexarchy/hooks/post-update.d/install-voxtype.hook"
...
[[ -f $test_home/.local/state/hexarchy/done/voxtype-install-invitation ]] || fail "Voxtype invitation records completion"
...
grep -qx 'exec:hexarchy-launch-floating-terminal-with-presentation hexarchy-voxtype-install' "$log_file" ||
