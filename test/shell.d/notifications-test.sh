...
assert(notifications.shouldBypassDnd({ appName: 'hexarchy-action', urgency: 1 }, 2), 'hexarchy action toasts bypass DND')
...
assert(!notifications.shouldBypassDnd({ appName: 'hexarchy-menu-keybindings', urgency: 1 }, 2), 'hexarchy command app names do not bypass DND')
assert(!notifications.isEphemeralApp('hexarchy-menu-keybindings'), 'notifications treat hexarchy command app names as normal apps')

// The click action's argv form: parsed from the persisted hexarchy-exec-argv
...
const execSnapshot = notifications.snapshotOf({
  id: 3,
  appName: 'hexarchy-action',
  summary: 'Download complete',
  hints: { 'hexarchy-exec-argv': '["mpv","--","/tmp/clip.mp4"]' }
}, 1)
assertEqual(
...
assertEqual(
  notifications.snapshotOf({ id: 3, hints: { 'hexarchy-glyph': '!' } }, 1).execArgv,
  '',
...
assert(
  ...
  'notifications service persists popups under the hexarchy state dir'
)
assert(
