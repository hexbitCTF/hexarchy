...
[[ ${HEXARCHY_TEST_HYPRCTL_FAILS:-0} == 1 ]] && exit 4
printf '%s\n' "$HEXARCHY_TEST_MONITORS"
SH
...
assert_status() {
  ...
  PATH="$fake_bin:$PATH" \
  HEXARCHY_TEST_MONITORS="$monitors" \
  HEXARCHY_TEST_HYPRCTL_FAILS="$hyprctl_fails" \
    "$ROOT/bin/hexarchy-hyprland-session-locked" || actual=$?
