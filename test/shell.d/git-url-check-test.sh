...
# hexarchy-git-url-check decides which URLs hexarchy-theme-install and
# hexarchy-plugin-add are willing to hand to `git clone`. git resolves a remote
...
check() {
  "$ROOT/bin/hexarchy-git-url-check" "$@" 2>&1
}
...
for url in "ext::sh -c id" "fd::0,1" "gcrypt::x" "a+b::x" "a.b::x" "a-b::x" "1::x"; do
  output=$(check "$url") &&
    fail "hexarchy-git-url-check refuses the transport helper '$url'" "$output"
  grep -qF "names a git option or transport helper" <<<"$output" ||
    fail "hexarchy-git-url-check names the helper rejection for '$url'" "$output"
done
...
for url in "ext://sh -c id" "fd://17" "gcrypt://example.com/x" "zzz://a" "ZZZ://a" "HTTPS://github.com/a/b"; do
  output=$(check "$url") &&
    fail "hexarchy-git-url-check refuses the '$url' transport" "$output"
  grep -qF "which Hexarchy does not clone from" <<<"$output" ||
    fail "hexarchy-git-url-check names the transport rejection for '$url'" "$output"
done
...
for url in "-x" "--upload-pack=touch /tmp/pwned" "-oProxyCommand=x"; do
  output=$(check "$url") &&
    fail "hexarchy-git-url-check refuses the option '$url'" "$output"
done

pass "a URL shaped like a git option is refused"

output=$(check "") && fail "hexarchy-git-url-check refuses an empty URL" "$output"
output=$(check) && fail "hexarchy-git-url-check refuses a missing URL" "$output"
...
for url in \
  ...
  "repo"; do
  output=$(check "$url") ||
    fail "hexarchy-git-url-check accepts the legitimate URL '$url'" "$output"
done
