...
cat >"$WORKDIR/bin/hexarchy-notification-send" <<SH
#!/bin/bash
...
receive() {
  ...
  : >"$WORKDIR/notifications"
  PATH="$WORKDIR/bin:$PATH" "$@" "$ROOT/bin/hexarchy-tailscale-receive" --once "$downloads"
...
[[ -z $(ls -A "$downloads/.hexarchy-taildrop") ]] ||
  fail "taildrop receive empties its staging directory" "$(ls -A "$downloads/.hexarchy-taildrop")"
