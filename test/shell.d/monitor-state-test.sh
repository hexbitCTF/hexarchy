...
cat >"$test_bin/hexarchy-brightness-display" <<'EOF'
#!/bin/bash
echo 42
EOF

cat >"$test_bin/hexarchy-hyprland-monitor-scaling" <<'EOF'
#!/bin/bash
echo 1.5
EOF
...
monitor_state() {
  ...
  mapfile -t state_lines < <(
    FAKE_MONITORS="$monitors_file" PATH="$test_bin:$PATH" \
      bash "$ROOT/bin/hexarchy-monitor-state" 2>/dev/null
  )
}
...
# Hexarchy mirrors by pointing the external at the internal, so `mirrorOf` lands
