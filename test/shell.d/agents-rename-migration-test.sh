...
cat >"$test_dir/bin/hexarchy-agent-usage-update" <<'STUB'
#!/bin/bash
...
mkdir -p "$home/.config/hexarchy" "$home/.cache/hexarchy/model-usage"
cat >"$config" <<'JSON'
{
  "bar": {
    "layout": {
      "center": ["hexarchy.model-usage"],
      "right": [
        { "id": "hexarchy.tray" },
        { "id": "hexarchy.model-usage", "syncMode": "On", "syncDir": "~/Sync/agent-usage" }
      ]
    }
  },
  "disabledPlugins": ["hexarchy.model-usage", "hexarchy.weather"]
}
JSON

run_migration

[[ $(jq -c '.bar.layout.right[1]' "$config") == '{"id":"hexarchy.agents","syncMode":"On","syncDir":"~/Sync/agent-usage"}' ]] ||
