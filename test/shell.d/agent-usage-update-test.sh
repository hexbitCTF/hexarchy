...
TEST_HOME=$(mktemp -d)
FAKE_HEXARCHY=$(mktemp -d)
trap 'rm -rf "$TEST_HOME" "$FAKE_HEXARCHY"' EXIT

mkdir -p "$FAKE_HEXARCHY/bin"

cat >"$FAKE_HEXARCHY/bin/hexarchy-agent-usage-good" <<'EOF'
#!/bin/bash
echo '{"schemaVersion":1,"id":"good","name":"Good Agent","totalPrompts":3}'
EOF

cat >"$FAKE_HEXARCHY/bin/hexarchy-agent-usage-noisy" <<'EOF'
#!/bin/bash
echo "this is not json"
EOF

cat >"$FAKE_HEXARCHY/bin/hexarchy-agent-usage-skipped" <<'EOF'
#!/bin/bash
echo '{"id":"skipped"}'
EOF

# The updater itself lives in the same namespace as the collectors it globs.
cat >"$FAKE_HEXARCHY/bin/hexarchy-agent-usage-update" <<'EOF'
#!/bin/bash
echo '{"id":"update"}'
EOF

chmod +x "$FAKE_HEXARCHY/bin/"hexarchy-agent-usage-*

usage_dir="$TEST_HOME/.local/state/hexarchy/agents/usage"

HOME="$TEST_HOME" HEXARCHY_PATH="$FAKE_HEXARCHY" XDG_STATE_HOME="" \
  "$ROOT/bin/hexarchy-agent-usage-update" --except skipped 2>/dev/null && fail "update reports a failing collector"
...
HOME="$TEST_HOME" HEXARCHY_PATH="$FAKE_HEXARCHY" XDG_STATE_HOME="" \
  "$ROOT/bin/hexarchy-agent-usage-update" skipped 2>/dev/null ||
