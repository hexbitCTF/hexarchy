...
run_pkg_prune() {
  PATH="$stub_bin:$PATH" "$ROOT/bin/hexarchy-update-pkg-prune"
}
...
# before the snapshot.
line_of() {
  grep -n "^[[:space:]]*$1\b" "$ROOT/bin/hexarchy-update" | head -1 | cut -d: -f1
}

prune_line=$(line_of hexarchy-update-pkg-prune)
snapshot_line=$(line_of hexarchy-snapshot)
pkgs_line=$(line_of hexarchy-update-system-pkgs)
[[ -n $prune_line && -n $snapshot_line && -n $pkgs_line ]] ||
  fail "hexarchy-update runs the cache prune, the snapshot, and the packages update"
