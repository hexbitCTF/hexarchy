...
for command in hexarchy-shell hyprctl pkill timeout; do
...
PATH="$mock_bin:$PATH" CALL_LOG="$call_log" "$ROOT/bin/hexarchy-system-lock"
...
[[ ${shutdown[2]} == "pkill -f [o]rg.hexarchy.screensaver" ]] ||
