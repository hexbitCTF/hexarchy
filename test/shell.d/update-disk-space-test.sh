...
write_stub hexarchy-snapshot '
touch "$SNAPSHOT_MARKER"
exit 0'

for command in \
  hexarchy-cmd-present \
...
TEST_AVAILABLE_BYTES=$((9 * 1024 * 1024 * 1024)) \
  PATH="$stub_bin:$ROOT/bin:$PATH" \
  "$ROOT/bin/hexarchy-update-requires-free-space" >/dev/null
status=$?
set -e
...
[[ $output == *"You need at least 10 GiB free to safely update Hexarchy."* ]] || fail "low disk space emits a warning"
...
[[ $output == *"You need at least 10 GiB free to safely update Hexarchy."* ]] || fail "interactive low-space update explains the requirement"
...
rm -f "$snapshot_marker" "$gum_marker"
output=$(HEXARCHY_UPDATE_FORCE=1 run_update -y)
