...
[[ -n ${HEXARCHY_TEST_QS_ARGS:-} ]] && printf '%s\n' "$*" >"$HEXARCHY_TEST_QS_ARGS"

if [[ ${HEXARCHY_TEST_QS_HANG:-0} == 1 ]]; then
  sleep 5
elif [[ ${HEXARCHY_TEST_QS_STARTING:-0} == 1 ]]; then
...
wrapper_error=$(PATH="$wrapper_bin:$PATH" \
  HEXARCHY_PATH="$wrapper_root" \
  HEXARCHY_SHELL_IPC_TIMEOUT=0.1s \
  HEXARCHY_TEST_QS_HANG=1 \
  "$ROOT/bin/hexarchy-shell" shell ping 2>&1) && fail "hung shell IPC returns a failure"
[[ $wrapper_error == "hexarchy-shell is not responding" ]] || fail "hung shell IPC reports that the shell is unresponsive" "$wrapper_error"
...
wrapper_error=$(PATH="$wrapper_bin:$PATH" \
  HEXARCHY_PATH="$wrapper_root" \
  HEXARCHY_TEST_QS_STARTING=1 \
  "$ROOT/bin/hexarchy-shell" shell ping 2>&1) && fail "a starting shell answers IPC calls with a failure"
[[ $wrapper_error == "hexarchy-shell is not ready" ]] || fail "a starting shell reports that it is not ready" "$wrapper_error"
pass "shell IPC calls fail while Quickshell is still starting"

PATH="$wrapper_bin:$PATH" \
HEXARCHY_PATH="$wrapper_root" \
HEXARCHY_TEST_QS_STARTING=1 \
  "$ROOT/bin/hexarchy-shell" -q shell ping >/dev/null 2>&1 ||
...
PATH="$wrapper_bin:$PATH" \
HEXARCHY_PATH="$wrapper_root" \
HEXARCHY_TEST_QS_ARGS="$wrapper_args" \
  "$ROOT/bin/hexarchy-shell" shell ping >/dev/null
...
ln -s "$ROOT/bin/hexarchy-shell" "$restart_bin/hexarchy-shell"
ln -s "$ROOT/bin/hexarchy-launch-shell" "$restart_bin/hexarchy-launch-shell"
ln -s "$ROOT/bin/hexarchy-cmd-missing" "$restart_bin/hexarchy-cmd-missing"
ln -s "$ROOT/bin/hexarchy-hyprland-session-locked" "$restart_bin/hexarchy-hyprland-session-locked"

cat >"$restart_bin/qs" <<'SH'
#!/bin/bash

printf '%s\n' "$*" >>"$HEXARCHY_TEST_IPC_LOG"

case "$*" in
  *'shell ping')
    [[ $* == *"-p $HEXARCHY_TEST_SESSION_PATH/shell"* ]] &&
      grep -Fx '303' "$HEXARCHY_TEST_QS_STATE" >/dev/null &&
      printf 'ok\n'
    ;;
  *'lock lock')
    touch "$HEXARCHY_TEST_QS_STATE.locked"
    printf 'ok\n'
    ;;
  *'lock status')
    if [[ -f $HEXARCHY_TEST_QS_STATE.locked ]]; then
...
case " $* " in
  *' kill -p '*)
    pid=$(head -n 1 "$HEXARCHY_TEST_QS_STATE")
    [[ $pid =~ ^[0-9]+$ ]] || exit 1
    kill "$pid" 2>/dev/null
    while kill -0 "$pid" 2>/dev/null; do sleep 0.01; done
    awk 'NR > 1' "$HEXARCHY_TEST_QS_STATE" >"$HEXARCHY_TEST_QS_STATE.next"
    mv "$HEXARCHY_TEST_QS_STATE.next" "$HEXARCHY_TEST_QS_STATE"
    ;;
  *' -n -p '*)
    printf '%s\n' "${HEXARCHY_TEST_TRANSIENT_ENV-unset}" >"$HEXARCHY_TEST_QS_ENV_LOG"
    printf '303\n' >"$HEXARCHY_TEST_QS_STATE"
    ;;
esac
SH
...
if [[ ${1:-} == "-j" && ${2:-} == "monitors" ]]; then
  ...
  if [[ ${HEXARCHY_TEST_SESSION_LOCKED:-0} == 1 ]]; then
  ...
  printf '%s\n' "${2:-}" >>"$HEXARCHY_TEST_DISPATCH_LOG"
  HEXARCHY_PATH="$HEXARCHY_TEST_SESSION_PATH" \
    env -u HEXARCHY_TEST_TRANSIENT_ENV hexarchy-launch-shell
  printf 'ok\n'
...
if [[ ${1:-} == "--user" && ${2:-} == "show-environment" ]]; then
  printf 'HEXARCHY_PATH=%s\n' "$HEXARCHY_TEST_SESSION_PATH"
...
HEXARCHY_PATH="$caller_root" \
XDG_RUNTIME_DIR="$runtime_dir" \
HEXARCHY_TEST_QS_STATE="$restart_state" \
HEXARCHY_TEST_QS_LOG="$restart_log" \
HEXARCHY_TEST_QS_ENV_LOG="$restart_env_log" \
HEXARCHY_TEST_DISPATCH_LOG="$dispatch_log" \
HEXARCHY_TEST_IPC_LOG="$ipc_log" \
HEXARCHY_TEST_SESSION_PATH="$restart_root" \
HEXARCHY_TEST_TRANSIENT_ENV=leaked \
  timeout 5 "$ROOT/bin/hexarchy-restart-shell"
...
grep -F 'hl.dsp.exec_cmd("hexarchy-launch-shell")' "$dispatch_log" >/dev/null || fail "restart launches the fresh shell through Hyprland"
...
locked_error=$(PATH="$restart_bin:$PATH" \
  HEXARCHY_PATH="$restart_root" \
  XDG_RUNTIME_DIR="$runtime_dir" \
  HEXARCHY_TEST_SESSION_LOCKED=1 \
  HEXARCHY_TEST_QS_STATE="$restart_state" \
  HEXARCHY_TEST_QS_LOG="$restart_log" \
  HEXARCHY_TEST_DISPATCH_LOG="$dispatch_log" \
  HEXARCHY_TEST_IPC_LOG="$ipc_log" \
  HEXARCHY_TEST_SESSION_PATH="$restart_root" \
  "$ROOT/bin/hexarchy-restart-shell" 2>&1) && fail "restart refuses while the shell lock is active"

[[ $locked_error == "Refusing to restart Hexarchy shell while the session is locked." ]] || fail "locked restart explains why it was refused" "$locked_error"
...
HEXARCHY_TEST_IPC_LOG="$ipc_log" \
HEXARCHY_TEST_SESSION_PATH="$restart_root" \
  timeout 5 "$ROOT/bin/hexarchy-restart-shell" || fail "locked restart recovers when the lock client is dead"
