...
run_application_bindings() {
  ...
  HOME="$home" XDG_CONFIG_HOME="$home/.config" XDG_STATE_HOME="$home/.local/state" HEXARCHY_PATH="$ROOT" HEXARCHY_BINDING_PRELUDE="$prelude" lua <<'LUA'
package.path = os.getenv("HOME") .. "/.config/?.lua;" .. os.getenv("HEXARCHY_PATH") .. "/?.lua;" .. package.path

local prelude = os.getenv("HEXARCHY_BINDING_PRELUDE") or ""
if prelude ~= "" then
  assert(load(prelude))()
end
...
mkdir -p "$removed_home/.local/state/hexarchy"
touch "$removed_home/.local/state/hexarchy/preinstalls-removed"
...
variable_output=$(run_application_bindings "$variable_home" 'hexarchy_preinstalled_bindings = false')
...
no_bindings_output=$(run_hexarchy_bindings "$no_bindings_home" 'hexarchy_default_bindings = false')
[[ -z $no_bindings_output ]] || fail "default binding variable disables all Hexarchy bindings" "$no_bindings_output"
pass "default binding variable disables all Hexarchy bindings"
...
voxtype_output=$(PATH="$voxtype_bin:$PATH" run_hexarchy_bindings "$voxtype_home")
...
voxtype_without_execute_output=$(PATH="$voxtype_bin:$PATH" run_hexarchy_bindings \
...
missing_voxtype_output=$(PATH="$missing_bin" run_hexarchy_bindings "$voxtype_home")
...
scratchpad_output=$(run_hexarchy_bindings "$scratchpad_home")
...
mkdir -p "$panels_home"
panels_output=$(run_hexarchy_bindings "$panels_home")
...
-- Application bindings without Hexarchy's preinstalled web apps, TUIs, or desktop apps.
...
LUA
HOME="$migration_home" HEXARCHY_PATH="$ROOT" bash -euo pipefail "$migration" >/dev/null
...
[[ -f $migration_home/.local/state/hexarchy/preinstalls-removed ]] ||
...
upgrade_script="$ROOT/bin/hexarchy-upgrade-to-quattro"
