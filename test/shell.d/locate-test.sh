...
HEXARCHY_UPDATEDB_CONF_PATH="$conf" bash -euo pipefail "$config_script" >/dev/null
...
HEXARCHY_UPDATEDB_CONF_PATH="$test_tmp/missing.conf" bash -euo pipefail "$config_script" >/dev/null
...
PATH="$fake_bin:$PATH" \
HEXARCHY_PATH="$ROOT" \
HEXARCHY_UPDATEDB_CONF_PATH="$conf" \
...
# checked-out tree may not have yet, and hexarchy-migrate runs under set -e.
: >"$test_tmp/calls.log"
