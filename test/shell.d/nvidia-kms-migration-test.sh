...
packaged_hooks="$ROOT/etc/mkinitcpio.conf.d/hexarchy_hooks.conf"
...
cat >"$stub_bin/hexarchy-cmd-present" <<'SH'
#!/bin/bash
...
hooks_conf="$test_tmp/hexarchy_hooks.conf"
...
run_migration() {
  PATH="$stub_bin:$PATH" \
    TEST_LOG="$calls" \
    HEXARCHY_MKINITCPIO_HOOKS_CONF="$hooks_conf" \
    HEXARCHY_MKINITCPIO_NVIDIA_CONF="$nvidia_conf" \
    HEXARCHY_KMS_REBUILD_MARKER="$rebuild_marker" \
    HEXARCHY_PCI_DEVICES_PATH="$test_tmp/devices" \
