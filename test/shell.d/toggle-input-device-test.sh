...
cat >"$stub_dir/hexarchy-osd" <<'EOF'
#!/bin/bash
:
EOF
chmod +x "$stub_dir/hexarchy-osd"

stub_device() {
  local kind=$1
  local name=$2
  cat >"$stub_dir/hexarchy-hw-$kind" <<EOF
#!/bin/bash
printf '%s\n' '$name'
EOF
  chmod +x "$stub_dir/hexarchy-hw-$kind"
}
...
run_toggle() {
  HOME="$home_dir" \
    ...
    "$ROOT/bin/hexarchy-toggle-input-device" "$@"
}
...
HOME="$home_dir" XDG_STATE_HOME="$xdg_decoy" HEXARCHY_PATH="$ROOT" MARKER="$marker" lua - <<'LUA'
local seen = {}
hl = {
...
HOME="$home_dir" XDG_STATE_HOME="$xdg_decoy" HEXARCHY_PATH="$ROOT" \
...
cat >"$stub_dir/hexarchy-hw-touchpad" <<'EOF'
#!/bin/bash
printf 'evil\nname\n'
EOF
chmod +x "$stub_dir/hexarchy-hw-touchpad"
...
reload_state="$reload_home/.local/state/hexarchy/toggles/hypr"
...
HOME="$reload_home" XDG_STATE_HOME="$reload_home/.local/state" HEXARCHY_PATH="$ROOT" lua - <<'LUA'
local disabled = {}
