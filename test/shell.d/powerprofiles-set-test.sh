...
export HEXARCHY_POWERPROFILES_STATE_DIR="$tmp_dir/state"

"$ROOT/bin/hexarchy-powerprofiles-set" ac balanced
...
"$ROOT/bin/hexarchy-powerprofiles-set" ac
[[ $(tail -n 1 "$tmp_dir/calls") == "balanced" ]] || fail "power profile restores AC preference"
pass "power profile restores AC preference"

if POWERPROFILES_SET_FAIL=1 "$ROOT/bin/hexarchy-powerprofiles-set" ac performance; then
...
"$ROOT/bin/hexarchy-powerprofiles-set" battery performance
...
ON_BATTERY=1 "$ROOT/bin/hexarchy-powerprofiles-set"
...
rm "$tmp_dir/state/ac"
ON_BATTERY=0 "$ROOT/bin/hexarchy-powerprofiles-set"
...
"$ROOT/bin/hexarchy-powerprofiles-set" ac power-saver
"$ROOT/bin/hexarchy-powerprofiles-init"
...
rg -F '["hexarchy-powerprofiles-set", pendingPowerSource]' "$ROOT/shell/plugins/services/battery/Service.qml" >/dev/null ||
  fail "battery service applies profiles through Hexarchy command"
pass "battery service applies profiles through Hexarchy command"

rg -F 'hexarchy-powerprofiles-set autodetect' "$ROOT/shell/plugins/menu/Menu.qml" >/dev/null ||
  fail "power profile menu persists selections through Hexarchy command"
pass "power profile menu persists selections through Hexarchy command"
