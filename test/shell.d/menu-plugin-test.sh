...
# The picker reads the plugin list from hexarchy-plugin-list and hands what it
...
cat >"$STUB_DIR/hexarchy-plugin-list" <<'STUB'
#!/bin/bash
cat "$FAKE_PLUGINS"
STUB

for command in hexarchy-plugin-enable hexarchy-plugin-disable; do
...
cat >"$STUB_DIR/hexarchy-menu-select" <<'STUB'
#!/bin/bash
cat >"$FAKE_ROWS"
printf '%s\n' "$FAKE_PICK"
STUB

cat >"$STUB_DIR/hexarchy-notification-send" <<'STUB'
#!/bin/bash
printf 'notification: %s\n' "$*" >>"$FAKE_CALLS"
STUB

cat >"$STUB_DIR/hexarchy-launch-floating-terminal-with-presentation" <<'STUB'
#!/bin/bash
printf 'terminal: %s\n' "$*" >>"$FAKE_CALLS"
STUB
...
pick() {
  ...
  HOME="$TMPDIR/home" \
    ...
    FAKE_PICK="$choice" \
    "$ROOT/bin/hexarchy-menu-plugin" "$verb" >/dev/null 2>&1
...
[
  {"id": "hexarchy.clock", "name": "Clock", "kinds": ["bar-widget"], "enabled": false, "active": false, "canDisable": true, "firstParty": true},
...
[[ $ROWS == *"$(printf 'Clock\thexarchy.clock')"* && $ROWS == *"$(printf 'Clock\ttester.clock')"* ]] \
  || fail "picker offers the id as subtext on every row" "$ROWS"
pass "picker offers the id as subtext on every row"
[[ $CALLS == *"hexarchy-plugin-enable tester.clock"* ]] \
...
[[ $CALLS == *"hexarchy-plugin-remove tester.clock"* ]] \
...
[[ $CALLS == *"hexarchy-plugin-enable acme.weather"* ]] \
...
cat >"$TMPDIR/plugins.json" <<'JSON'
[
  {"id": "hexarchy.clock", "name": "Clock", "kinds": ["bar-widget"], "enabled": true, "active": false, "canDisable": true, "firstParty": true},
  {"id": "acme.weather", "name": "Weather", "kinds": ["bar-widget"], "enabled": false, "active": false, "canDisable": true, "firstParty": false}
]
JSON

pick clone "$(printf 'Clock\thexarchy.clock')"
[[ $ROWS == *"Clock"* && $ROWS != *"Weather"* ]] ||
  fail "clone picker offers only built-in plugins" "$ROWS"
pass "clone picker offers built-in plugins"
[[ $CALLS == *"terminal: hexarchy-plugin-clone hexarchy.clock --edit"* ]] ||
...
[
  {"id": "hexarchy.clock", "name": "Clock", "kinds": ["bar-widget"], "enabled": true, "active": false, "canDisable": true, "firstParty": true},
  {"id": "tester.clock", "name": "My Clock", "kinds": ["bar-widget"], "enabled": false, "active": false, "canDisable": true, "firstParty": false, "clonedFrom": "hexarchy.clock"}
]
JSON
...
[[ $CALLS == *"hexarchy-plugin-enable acme.fancy"* && $CALLS != *"--section"* ]] \
...
pick disable "$(printf 'Clock\thexarchy.clock')"
...
cat >"$TMPDIR/plugins.json" <<'JSON'
[
  {"id": "hexarchy.bar", "name": "Bar", "kinds": ["bar"], "enabled": false, "active": false, "canDisable": false, "firstParty": true},
  {"id": "tester.neon-bar", "name": "Neon Bar", "kinds": ["bar"], "enabled": true, "active": true, "canDisable": false, "firstParty": false}
]
JSON

pick enable "$(printf 'Bar\thexarchy.bar')"
...
[[ $CALLS == *"hexarchy-plugin-enable hexarchy.bar"* ]] \
...
cat >"$TMPDIR/plugins.json" <<'JSON'
[
  {"id": "hexarchy.bar", "name": "Bar", "kinds": ["bar"], "enabled": true, "active": true, "canDisable": false, "firstParty": true}
]
JSON
