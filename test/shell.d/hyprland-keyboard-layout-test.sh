...
resolved_input() {
  HEXARCHY_PATH="$ROOT" HEXARCHY_VCONSOLE="${1-}" lua <<'LUA'
package.path = os.getenv("HEXARCHY_PATH") .. "/?.lua;" .. package.path

local vconsole = os.getenv("HEXARCHY_VCONSOLE")
...
hooks_conf="$ROOT/etc/mkinitcpio.conf.d/hexarchy_hooks.conf"
...
[[ -n $hooks_layouts ]] || fail "non-latin layout list is readable from hexarchy_hooks.conf"
