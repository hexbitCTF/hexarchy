...
const defaultMenuJsonc = fs.readFileSync(path.join(root, 'default/hexarchy/hexarchy-menu.jsonc'), 'utf8')
...
const installed = menu.normalizeItem('install.browser.zen', { label: 'Zen', disabled: 'hexarchy-pkg-present zen-browser-bin', action: 'install-zen' })
...
assert(
  defaultById['update.hexarchy'].icon === '\ue900',
  'menu update Hexarchy entry uses the Hexarchy glyph'
)
assert(
  defaultById['update.hexarchy'].iconFont === 'hexarchy',
  'menu update Hexarchy entry renders the private glyph with the Hexarchy font'
)
assertEqual(
  defaultById['update.themes'].when,
  'hexarchy-theme-extras',
...
assert(
  defaultById['setup.direct-boot'].action.includes('hexarchy-setup-direct-boot'),
  'menu places Direct Boot directly under Setup'
)
assert(
  defaultById['setup.reset'].action.includes('hexarchy-system-factory-reset'),
...
const expectedAgents = {
  ...
  pi: { icon: '\ue901', iconFont: 'hexarchy', label: 'Pi' },
  omp: { icon: '\ue903', iconFont: 'hexarchy', label: 'omp' },
  opencode: { icon: '\ue902', iconFont: 'hexarchy', label: 'OpenCode' },
  ori: { icon: '\ue909', iconFont: 'hexarchy', label: 'Ori' },
  claude: { icon: '󰛄', label: 'Claude' },
  codex: { icon: '\ue905', iconFont: 'hexarchy', label: 'Codex' },
  grok: { icon: '\ue904', iconFont: 'hexarchy', label: 'Grok' },
...
assert(
  Object.entries(expectedAgents).every(([agent, expected]) => {
    ...
    return entry
      ...
      && entry.action === `hexarchy-default-agent ${agent}`
      && !entry.when
...
// dropped, so the list reads as a catalog of what Hexarchy can install.
...
assertEqual(
  ...
  'hexarchy-pkg-present zen-browser-bin',
...
assert(
  defaultItems.filter(item => item.id.startsWith('remove.')).every(item => !item.disabled)
    && defaultById['remove.browser.zen'].when === 'hexarchy-pkg-present zen-browser-bin',
...
assert(
  defaultById['setup.security.passwordless-sudo'].action.includes('hexarchy-sudo-passwordless'),
...
assert(
  ['top', 'bottom', 'left', 'right'].every(position => defaultById[`style.bar.position.${position}`].action === `hexarchy-bar position ${position}`),
  'menu lists all Menu Bar positions under Position'
)
assertEqual(
  defaultById['style.bar.transparency'].action,
  'hexarchy-bar transparent toggle',
...
assert(
  ['enable', 'disable', 'clone', 'remove'].every(
    verb => defaultById[`setup.plugin.${verb}`].action === `hexarchy-menu-plugin ${verb}`
  ),
...
assert(
  defaultById['setup.plugin.remove'].when.includes('.config/hexarchy/plugins'),
  'menu hides Remove until a plugin the user installed exists to delete'
)
assert(
  defaultById['setup.plugin.add'].action.includes('hexarchy-plugin-add'),
  'menu adds a plugin through the CLI, where the trust warning and clone output are visible'
)

const pluginPicker = fs.readFileSync(path.join(root, 'bin/hexarchy-menu-plugin'), 'utf8')
assert(
...
const pluginAdd = fs.readFileSync(path.join(root, 'bin/hexarchy-plugin-add'), 'utf8')
const pluginEnable = fs.readFileSync(path.join(root, 'bin/hexarchy-plugin-enable'), 'utf8')
assert(
  /Now using \$id as the bar/.test(pluginEnable)
    && /hexarchy-plugin-enable "\$id" "\$\{ENABLE_PLACEMENT\[@\]\}"/.test(pluginAdd),
...
assert(
  /"hexarchy-plugin-\$1" "\$id"/.test(pluginPicker),
...
assert(
  /hexarchy-launch-floating-terminal-with-presentation "hexarchy-plugin-remove/.test(pluginPicker),
...
font_charset=$(fc-query --format='%{charset}' "$ROOT/default/fonts/hexarchy/hexarchy.ttf")
[[ $font_charset == *"e900-e909"* ]] || fail "Hexarchy icon font includes every custom menu glyph"
pass "Hexarchy icon font includes the official agent marks"
