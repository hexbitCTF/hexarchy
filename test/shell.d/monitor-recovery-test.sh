...
monitor_watch="$ROOT/bin/hexarchy-hyprland-monitor-watch"
monitor_internal="$ROOT/bin/hexarchy-hyprland-monitor-internal"
monitor_mirror="$ROOT/bin/hexarchy-hyprland-monitor-internal-mirror"
monitor_laptop="$ROOT/bin/hexarchy-hyprland-monitor-laptop"
monitor_external_active="$ROOT/bin/hexarchy-hyprland-monitor-external-active"
system_wake="$ROOT/bin/hexarchy-system-wake"
clamshell="$ROOT/bin/hexarchy-hyprland-monitor-clamshell"
lock_service="$ROOT/shell/plugins/lock/Service.qml"
hw_clamshell="$ROOT/bin/hexarchy-hw-clamshell"
hw_laptop_closed="$ROOT/bin/hexarchy-hw-laptop-closed"
...
grep -F 'hexarchy-hyprland-monitor-clamshell' "$monitor_watch" >/dev/null
...
grep -F 'hexarchy-hw-laptop && hexarchy-hyprland-monitor-external-active' "$monitor_watch" >/dev/null
...
grep -F 'hexarchy-hyprland-reload-guard paused' "$monitor_watch" >/dev/null
pass "modeless recovery does not reload into a package transaction"

grep -F '.disabled != true and (.width == 0 or .height == 0)' "$ROOT/bin/hexarchy-hyprland-monitor-modeless" >/dev/null
grep -F 'hyprctl monitors all -j' "$ROOT/bin/hexarchy-hyprland-monitor-modeless" >/dev/null
pass "modeless helper sees mirrors and ignores monitors disabled on purpose"

grep -F 'hexarchy-hw-laptop-closed && hexarchy-hw-external-monitors' "$hw_clamshell" >/dev/null
...
grep -F 'hexarchy-hyprland-monitor-internal recover >/dev/null 2>&1 || true' "$clamshell" >/dev/null
grep -F 'hexarchy-hyprland-monitor-internal-mirror recover >/dev/null 2>&1 || true' "$clamshell" >/dev/null
...
grep -F 'hexarchy-hyprland-monitor-external-active' "$clamshell" >/dev/null
grep -F 'hexarchy-hw-clamshell' "$clamshell" >/dev/null
...
grep -F 'hexarchy-hyprland-monitor-laptop' "$monitor_internal" >/dev/null
grep -F 'hyprctl monitors all -j' "$monitor_laptop" >/dev/null
grep -F 'hexarchy-hyprland-monitor-external-active' "$monitor_internal" >/dev/null
grep -F 'wake' "$monitor_internal" >/dev/null
grep -F 'hexarchy-hyprland-toggle-enabled $TOGGLE || return 0' "$monitor_internal" >/dev/null
...
grep -F 'hexarchy-hyprland-monitor-external-active' "$monitor_mirror" >/dev/null
pass "internal mirror helper recovers when no active external display remains"

grep -F 'switch:on:Lid Switch", nil, "hexarchy-system-lid-close"' "$utilities" >/dev/null
grep -F 'switch:off:Lid Switch", nil, "hexarchy-hyprland-monitor-clamshell"' "$utilities" >/dev/null
pass "lid switch bindings lock on close and reconcile clamshell display state"

grep -F 'hexarchy-hyprland-monitor-clamshell >/dev/null 2>&1 || true' "$system_wake" >/dev/null
