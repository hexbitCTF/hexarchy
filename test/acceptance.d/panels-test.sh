...
WEATHER_FILE="$HOME/.local/state/hexarchy/settings/weather.json"
...
hide_panels() {
  ...
  for plugin in hexarchy.weather hexarchy.bluetooth hexarchy.network hexarchy.audio hexarchy.monitor hexarchy.power; do
    hexarchy-shell shell hide "$plugin" >/dev/null 2>&1 || true
  done
}
...
open_and_capture_panel() {
  ...
  wait_until "$name panel opens" 15 layer_present "hexarchy-keyboard-panel"
  sleep 1
  screenshot "success-panel-$name"

  hexarchy-shell shell hide "$plugin" >/dev/null
  wait_until "$name panel closes" 15 layer_absent "hexarchy-keyboard-panel"
}
...
hexarchy-weather-location --set "San Francisco" "37.7749,-122.4194"
hexarchy-shell shell summon hexarchy.weather >/dev/null
wait_until "weather panel opens" 15 layer_present "hexarchy-keyboard-panel"
...
hexarchy-shell shell hide hexarchy.weather >/dev/null
wait_until "weather panel closes" 15 layer_absent "hexarchy-keyboard-panel"

status=0
panels='bluetooth|hexarchy.bluetooth
network|hexarchy.network
audio|hexarchy.audio
monitor|hexarchy.monitor'

while IFS='|' read -r name plugin; do
  if ! (trap - EXIT; open_and_capture_panel "$name" "$plugin"); then
    status=1
    hide_panels
    wait_until "$name failed panel is dismissed" 15 layer_absent "hexarchy-keyboard-panel"
  fi
done <<<"$panels"
...
if upower -e | grep '/battery_' >/dev/null; then
  if ! (trap - EXIT; open_and_capture_panel "power" "hexarchy.power"); then
    status=1
    hide_panels
    wait_until "power failed panel is dismissed" 15 layer_absent "hexarchy-keyboard-panel"
  fi
else
...
hexarchy-shell shell summon hexarchy.bluetooth >/dev/null
...
wtype -k Tab
sleep 2
wait_until "Tab keeps a shell panel open" 15 layer_present "hexarchy-keyboard-panel"
screenshot "success-panel-navigation-02-next"
hide_panels
wait_until "keyboard-navigated panel closes" 15 layer_absent "hexarchy-keyboard-panel"
...
hexarchy-shell shell summon hexarchy.bluetooth >/dev/null
wait_until "focus-prime panel opens" 15 layer_present "hexarchy-keyboard-panel"
if (( $(hyprctl -j monitors | jq length) == 1 )); then
  layer_absent "hexarchy-keyboard-panel-dismiss" || fail "single-monitor panel has no dismissal twin"
  pass "single-monitor panel has no dismissal twin"
fi
hexarchy-shell shell hide hexarchy.bluetooth >/dev/null
hexarchy-shell shell summon hexarchy.bluetooth >/dev/null
wait_until "focus-prime panel reopens" 15 layer_present "hexarchy-keyboard-panel"
sleep 1
screenshot "success-panel-focus-prime-reopened"
wtype -k Escape
wait_until "Escape closes a panel reopened during fade" 15 layer_absent "hexarchy-keyboard-panel"
