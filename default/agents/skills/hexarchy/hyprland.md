...
Hexarchy configures Hyprland in Lua. User files are loaded after Hexarchy's
defaults, so overrides go here:

```
~/.config/hypr/
├── hyprland.lua       # Main config (loads Hexarchy defaults, then user files)
...
- Use `hexarchy refresh hyprland` to reset the Lua config files to defaults

The two `.conf` files are read by separate processes, so `hyprctl` neither
applies nor validates them:
- `hyprsunset.conf` (night light): apply changes with `hexarchy restart hyprsunset`; reset with `hexarchy refresh hyprsunset`
...
View current bindings: `hexarchy menu keybindings --print`

**IMPORTANT: When re-binding an existing key:**

1. First check existing bindings: `hexarchy menu keybindings --print`
...
Window rules go in `~/.config/hypr/hyprland.lua` or a required Lua module. Prefer Hexarchy's `o.window(match, rules)` helper — see examples in `$HEXARCHY_PATH/default/hypr/windows.lua`.
