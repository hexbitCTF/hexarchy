...
Read this before adding a branded glyph to `default/fonts/hexarchy/hexarchy.ttf`.

The Hexarchy icon font is a small private-use font carrying the marks Nerd
Fonts does not have: the Hexarchy logo and the agent and app brand marks. The
menu draws one by naming the font on an entry:

```jsonc
"setup.default.agent.grok": {"icon":"","iconFont":"hexarchy","label":"Grok", ...}
```
...
`default/fonts/hexarchy/README.md` lists every glyph with the URL its artwork
came from. Keep that list accurate — it is the only record of provenance.

## Adding a glyph

`hexarchy dev font` does the work:

```bash
hexarchy dev font list
hexarchy dev font add ollama https://simpleicons.org/icons/ollama.svg
```
...
- Point the menu entry at the new codepoint with `"iconFont":"hexarchy"`.
...
The font is package-owned: `hexarchy-settings` installs it to
`/usr/share/fonts/hexarchy/hexarchy.ttf`, so a new glyph reaches the desktop
through a settings release, not through `hexarchy update`. Between the merge and
...
magick -background white -fill black -font default/fonts/hexarchy/hexarchy.ttf \
...
`hexarchy menu refresh` alone will not pick up a changed font.
