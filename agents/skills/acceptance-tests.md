...
`test/acceptance.d/*-test.sh`. It exercises a real installed Hexarchy desktop,
...
Run acceptance tests in a disposable VM through the sibling `hexarchy-iso`
...
```bash
cd ../hexarchy-iso
./bin/hexarchy-iso-test release/<iso>.iso --reuse-base --sync-hexarchy ../hexarchy --no-preview
```

Use `--sync-all ../hexarchy` instead of `--sync-hexarchy ../hexarchy` when the
...
```bash
cd ../hexarchy-iso
./bin/hexarchy-iso-make --no-boot-offer --local-source ../hexarchy ../hexarchy-pkgs
./bin/hexarchy-iso-test release/<generated-iso>.iso --no-preview
```
