...
Run `hexarchy-restart-shell` after making changes to QML files.
...
- First-party plugins live directly under `shell/plugins/` or one category
  ...
  at `~/.config/hexarchy/plugins/<id>/` with a `manifest.json` at the root.
- Every plugin manifest declares `schemaVersion`, `id`, `name`, `version`,
  `kinds`, and `entryPoints`. See
  [`docs/hexarchy-shell.md`](../../docs/hexarchy-shell.md) and
...
- Entry-point QML files are `Item`s (not `ShellRoot`), and accept the
  shell-injected properties `hexarchyPath`, `shell`, `manifest`, and
...
- `bin/hexarchy-shell` is the canonical IPC entry point. It forwards to
...
- The `shell` IPC target exposes lifecycle and configuration methods including
  ...
  `image-selector`, which drives the `hexarchy.image-picker` panel.
- Individual plugins register their own IPC targets, named for the plugin rather
  ...
  bar widgets register one target each — `hexarchy.indicators`,
  `hexarchy.system-update`, `hexarchy.clock`. There is no `bar` target.
