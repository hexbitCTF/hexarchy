...
ShellRoot {
  ...
  // The hexarchy-shell host is the long-running entry point. Plugins live in
  // sibling directories under plugins/. HEXARCHY_PATH is provided by the uwsm
  ...
  readonly property string shellPath: hexarchyPath + "/shell"
  readonly property string firstPartyPluginsDir: shellPath + "/plugins"
  readonly property string defaultsPath: hexarchyPath + "/config/hexarchy/shell.json"
  readonly property string userConfigPath: home + "/.config/hexarchy/shell.json"
  ...
  readonly property var builtinShellConfig: ({
    ...
    bar: {
      ...
      layout: {
        ...
        center: [{ id: "hexarchy.clock", format: "dddd HH:mm" }],
  ...
  Component.onCompleted: {
    console.log("hexarchy-shell paths",
      "hexarchyPath=" + shell.hexarchyPath,
  ...
  readonly property string defaultBarId: "hexarchy.bar"
  ...
  function configureBar(target, manifest) {
    if (!target) return
    if ("hexarchyPath" in target) target.hexarchyPath = shell.hexarchyPath
  ...
  function ensureService(pluginId) {
    ...
    function finalize() {
      ...
      if ("hexarchyPath" in inst) inst.hexarchyPath = shell.hexarchyPath
  ...
  function isBarWidgetPanelPlugin(pluginId) {
    ...
    // panel loader (e.g. hexarchy.menu); let that path handle them.
  ...
  Instantiator {
    ...
    delegate: QtObject {
      ...
      property Loader panelLoader: Loader {
        ...
        onLoaded: {
          if (!item) return
          if ("hexarchyPath" in item) item.hexarchyPath = shell.hexarchyPath
  ...
  function imagePickerItem() {
    var loader = panelLoaders["hexarchy.image-picker"]
  ...
  IpcHandler {
    ...
    function open(imageDirs: string,
      ...
      return shell.summon("hexarchy.image-picker", payload) ? "ok" : "unknown"
    }
    ...
    function cancel(doneFile: string): string {
      ...
      if (picker && typeof picker.closeSelector === "function") {
        ...
        shell.hide("hexarchy.image-picker")
      }
      return "ok"
    }
  ...
  IpcHandler {
    ...
    function listPlugins(): string {
      ...
      for (var id in plugins) {
        ...
        var metadata = plugins[id].hexarchy
        var clonedFrom = Util.isPlainObject(metadata) ? String(metadata.clonedFrom || "") : ""
        out.push({
          id: id,
          name: plugins[id].name,
          kinds: kinds,
          // What `hexarchy plugin enable/disable` toggles: for a widget that is
