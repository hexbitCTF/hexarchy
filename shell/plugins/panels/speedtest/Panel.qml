...
// hexarchy-network-status when the caller doesn't know it.
Item {
  id: root
  ...
  function dismiss() {
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "hexarchy.speedtest")
    else close()
  }
  ...
  function startPhase(nextPhase) {
    ...
    phase = nextPhase
    stderrText = ""
    speedTestProc.command = ["hexarchy-network-speedtest", nextPhase]
  ...
  Process {
    id: statusProc
    command: ["hexarchy-network-status"]
  ...
  SpeedTestOverlay {
    ...
    layerNamespace: "hexarchy-network-speedtest"
