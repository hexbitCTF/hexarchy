...
// hexarchy-agent-usage-update, which writes one JSON record per agent into
...
Item {
  ...
  readonly property string usageDir: (Quickshell.env("XDG_STATE_HOME") || home + "/.local/state") + "/hexarchy/agents/usage"
  ...
  function updateCommand(kind, agentIds) {
    var command = ["hexarchy-agent-usage-update"]
  ...
  readonly property string syncSnapshotPath: syncConfigured() ? syncEffectiveDir + "/" + syncEffectiveFileName : home + "/.cache/hexarchy/agents-disabled.json"
  ...
  // Snapshots keep the field names older Hexarchy versions wrote, so a fleet
