...
`hexarchy-agent-usage-update` writes to `~/.local/state/hexarchy/agents/usage/`
...
`hexarchy plugin disable hexarchy.agents`.

## Data

Each agent is one JSON record in `~/.local/state/hexarchy/agents/usage/`,
written by `hexarchy-agent-usage-update`. That command runs one
`hexarchy-agent-usage-<agent>` collector per agent; the widget invokes it
...
configuration in `~/.config/hexarchy/agents/fireworks.json`:
...
- IPC: `hexarchy-shell hexarchy.agents <open|close|toggle|refresh|next>`.

## Settings

Settings live in the widget's entry in `~/.config/hexarchy/shell.json`. The
top-level keys can be set with
`hexarchy bar set hexarchy.agents <key> <value>`:
...
```bash
hexarchy bar set hexarchy.agents refreshIntervalSec 300 --json
hexarchy bar set hexarchy.agents syncDir '~/Sync/agent-usage'
```
...
```bash
hexarchy bar set hexarchy.agents providers '{
  "claude": { "enabled": true },
  "codex": { "enabled": false },
