...
Item {
  ...
  // The hexarchy-shell host injects hexarchyPath from HEXARCHY_PATH.
  required property string hexarchyPath
  ...
  property string hexarchyConfigDir: home + "/.config/hexarchy"
  ...
  // to "monospace"), so changing the system font (via `hexarchy-font-set`)
  ...
  // Resolve the live bar-widget instance for a plugin id (e.g. "hexarchy.bluetooth").
  ...
  function customModuleSource(entry) {
    var source = BarModel.customModulePath(entry, home, hexarchyConfigDir)
  ...
  FileView {
    path: root.stateHome + "/hexarchy/current"
    watchChanges: true
    printErrors: false
  ...
  // and the flag is created/removed by `hexarchy-toggle-bar`.
  Process {
    id: barHiddenProbe
    running: true
    command: ["bash", "-c", "[[ -f $HOME/.local/state/hexarchy/toggles/bar-off ]] && echo yes || echo no"]
    stdout: SplitParser { onRead: function(line) { root.barHidden = String(line).trim() === "yes" } }
  }
  FileView {
    path: root.home + "/.local/state/hexarchy/toggles"
    watchChanges: true
    printErrors: false
  ...
  component BarPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "hexarchy-bar"
  ...
  component DragGhostPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "hexarchy-bar-drag-ghost"
  ...
  component BarMoveGhostPanel: PanelWindow {
    ...
    color: "transparent"
    exclusionMode: ExclusionMode.Ignore
    WlrLayershell.namespace: "hexarchy-bar-move-ghost"
    WlrLayershell.layer: WlrLayer.Overlay
    WlrLayershell.keyboardFocus: WlrKeyboardFocus.None
