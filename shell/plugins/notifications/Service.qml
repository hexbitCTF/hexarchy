// Notification service for the hexarchy shell.
...
Item {
  ...
  readonly property string stateDir: home + "/.local/state/hexarchy/"
  ...
  // Toasts are fixed to the top-right corner. They only clear the hexarchy bar
  ...
  PersistentProperties {
    id: persisted
    reloadableId: "hexarchy-notifications"
  ...
  // and rare.
  //   - hexarchy-action: a user-action confirmation toast ("Theme changed",
  ...
  //     Trusted because it's almost always hexarchy or system shell scripts —
  ...
  //   - app_name is "hexarchy-action" (Hexarchy's own user-action toasts —
  ...
  // Run the popup's click action, then dismiss. Hexarchy's own toasts carry the
  ...
  function focusApp(entry) {
    ...
    focusAppProc.command = [
      service.hexarchyPath + "/bin/hexarchy-hyprland-focus-app",
      String(entry.app)
    ]
  ...
  // toasts survive shell restarts (notably the restart `hexarchy-update`
  ...
  Variants {
    ...
    PanelWindow {
      ...
      WlrLayershell.namespace: "hexarchy-notifications"
