...
Item {
  ...
  // Injected by hexarchy-shell (the first-party service loader).
  property var shell: null

  readonly property string home: Quickshell.env("HOME")
  readonly property string stayAwakeStateDir: home + "/.local/state/hexarchy/indicators"
  ...
  readonly property string screensaverClass: "org.hexarchy.screensaver"
  ...
  function logEvent(event, details) {
    ...
    console.log("hexarchy idle " + root.lastEventAt + " " + root.lastEvent)
  }
  ...
  function launchScreensaver() {
    ...
    runProcess(screensaverProcess, "screensaver", "[[ $(hexarchy-shell lock isLocked 2>/dev/null) == \"true\" ]] || hexarchy-launch-screensaver")
  }
  ...
  function lockSystem(reason) {
    ...
    runProcess(lockProcess, "lock", "hexarchy-system-lock")
  }
  ...
  function cancelIdleCycle(reason) {
    ...
    if (root.idledThisCycle) runProcess(wakeProcess, "wake", "hexarchy-system-wake")
  ...
  function persistStayAwake(value) {
    var command = value
      ? "mkdir -p \"$HOME/.local/state/hexarchy/indicators\" && touch \"$HOME/.local/state/hexarchy/indicators/stay-awake\""
      : "rm -f \"$HOME/.local/state/hexarchy/indicators/stay-awake\""
  ...
  Process {
    ...
    command: ["bash", "-c", "mkdir -p \"$HOME/.local/state/hexarchy/indicators\"; if [[ -f $HOME/.local/state/hexarchy/indicators/stay-awake ]]; then echo yes; else echo no; fi"]
    stdout: SplitParser {
