...
QtObject {
  ...
  // component follows the fontconfig alias `hexarchy-font-set` writes.
  ...
  // alias path keeps working when the user runs `hexarchy font set`), but
  ...
  // HEXARCHY_MENU_FONT override for users who want a different family on the
  ...
  readonly property string menuFontFamily: {
    var override = Quickshell.env("HEXARCHY_MENU_FONT")
  ...
  // Resolve the fontconfig alias to a concrete family name. `hexarchy font
  ...
  // `hexarchy toggle window-gaps` creates/removes this flag file. Hyprland
  ...
  property FileView windowNoGapsToggle: FileView {
    path: Quickshell.env("HOME") + "/.local/state/hexarchy/toggles/hypr/window-no-gaps.lua"
    watchChanges: true
    printErrors: false
