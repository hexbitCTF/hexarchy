...
QtObject {
  ...
  readonly property string currentThemePath: stateHome + "/hexarchy/current/theme"
  ...
  // Style. User keys win, so a machine-level `~/.config/hexarchy/shell.toml`
  ...
  // is where `hexarchy display text size` writes `[font] base-size`. Watched so the
  // CLI takes effect live without restarting the shell; absent by default.
  property FileView userShellFile: FileView {
    id: userShellFile
    path: root.home + "/.config/hexarchy/shell.toml"
    watchChanges: true
    printErrors: false
