...
that hosts the Hexarchy desktop. The bar, panels, overlays, menus, and
services all run inside as plugins. Hyprland autostart launches the
shell via `hexarchy-launch-shell`; restart it with `hexarchy-restart-shell`.
IPC is the canonical way for CLIs to talk to a running shell —
`hexarchy-shell` forwards a call and fails when the shell is not running
(`-q` makes it quiet best-effort; `HEXARCHY_SHELL_IPC_TIMEOUT` bounds the
wait).
...
| `bar`        | Full bar option that can replace `hexarchy.bar` |
...
Only one full bar option is active at a time. The built-in `hexarchy.bar` is
...
`hexarchyPath`, `shell`, `manifest`, and the registries (`pluginRegistry` /
...
clones it straight into `~/.config/hexarchy/plugins/<id>/`; updating is a
fast-forward pull:

```bash
hexarchy plugin add https://github.com/acme/hexarchy-weather.git
hexarchy plugin update                # fetches, shows a diff, fast-forwards
...
Cloning `hexarchy.clock`, for example, creates and switches to
`~/.config/hexarchy/plugins/<username>.clock/` (e.g. `dhh.clock`), names it
...
Plugins run as **unsandboxed code** inside `hexarchy-shell`. Adding warns you
before cloning, plugins land disabled so you can review the code before
`hexarchy plugin enable`, and updates show a diff before touching anything.
...
`~/.config/hexarchy/plugins/<id>/`, run `hexarchy-shell shell rescanPlugins`, then
`hexarchy plugin enable <id>`. A bar widget starts in its declared default
section; enabling a full bar replaces the one in use. `hexarchy bar` drives the
bar from the CLI — `use | reset | defaults | position | transparent | put |
move | set`, with placement flags such as `--section` and `--index`.
The lower-level IPC methods remain available through `hexarchy-shell shell ...`.
...
`media`, `notifications`, and per-widget targets such as `hexarchy.clock`
or `hexarchy.power`. There is no `bar` target.
...
| `putBarWidget <id> <placementJson>`   | place a widget only where absent (`hexarchy bar put`) |
...
1. The active bar option is `bar.id`. Omit it or set it to `hexarchy.bar` for
...
3. Settings are inline on the entry. No `config:` sub-object, no
   merge layers.
4. Built-in bar widget ids are namespaced (`hexarchy.clock`, `hexarchy.audio`, …).
...
`config/hexarchy/shell.json` describes the fresh-install state. When no
...
machine-level `~/.config/hexarchy/shell.toml` is watched live by the
shell and its keys win over the active theme's `shell.toml`, so
overrides like `hexarchy display text size` survive theme switches.
...
`hexarchy-theme-set-templates` (see [`theming.md`](theming.md)).
...
themes don't set it, the user does via `hexarchy font set <name>`.
...
```json
{ "id": "vpn", "type": "command", "exec": "~/.config/hexarchy/bar/scripts/vpn-status",
...
Then `~/.config/hexarchy/bar/modules/gpu.qml` (or set `source` to point
