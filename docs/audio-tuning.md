...
`on` renders the graph into `~/.config/pipewire/hexarchy-speaker-tuning.conf.d/`
(plus the shared host config `hexarchy-speaker-tuning.conf`, from
`default/audio/filter-chain-host.conf`) and runs it as its own PipeWire client via
`hexarchy-speaker-tuning.service`, rather than
...
```bash
hexarchy audio tuning on        # install the matching tuning
hexarchy audio tuning off       # remove it, back to raw speakers
hexarchy audio tuning status    # installed? in use? what matches?
```
...
node is named `hexarchy_speaker_tuning` (the install checks for that exact name,
...
tools, in [hexarchy-audio-tuner](https://github.com/omacom-io/hexarchy-audio-tuner).
It is not installed by default — almost nobody authoring a tuning, and it needs
python, ffmpeg and mpv.

```bash
hexarchy pkg add hexarchy-audio-tuner
```
...
- **Volume lives downstream of the tuning.** A tuning is a virtual sink that
  ...
  `hexarchy-audio-output-sink` is the single definition of "which sink does this
...
- **The fronted sink is hidden.** The tuning and the physical speakers both exist
  ...
  `hexarchy-audio-sink-availability` reports it unavailable and
  `hexarchy-audio-output-switch` skips it, leaving one speaker entry in the panel.
...
- **The tuning's own output must not be moved.** A filter-chain's output is a
  ...
  `hexarchy-audio-output-set-default` moves only streams that carry an
  `application.name`.
